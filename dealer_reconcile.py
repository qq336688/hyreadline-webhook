...
from app import sb, now_str, admin_required, _send_notification_mail  # noqa: 延後匯入，app.py 需在 register_blueprint 前已定義好 sb
...
def bookstore_invoice_required(f):
    """2026-08-12新增：「上傳團體訂單含發票」專用權限，跟「方案清單」「匯入訂單」都各自獨立
    （見 mod_bookstore_invoice_view，dealer_reconcile_schema.sql v1.20）。這個功能純粹是
    團體訂單資訊的登記/查詢，不跟訂單或方案清單的拆帳資料比對，所以刻意不沿用
...
def _map_sku_header(h):
    ...
    if 'EAN' in h.upper():
        return 'ean'
    if '出貨商品Id' in h:  # 2026-08-25新增：出貨系統「商品編號」，比對可靠度比商品名稱高
        return 'shipping_system_product_id'
    if '出貨系統商品名稱' in h:  # 2026-08-25新增：供未來「出庫數比對未開發票數」功能比對用
        return 'shipping_system_product_name'
    if '發票總表比對名稱' in h:  # 2026-09-11新增：見shipping_system_product_name同一套設計，
        return 'ledger_history_sku_name'  # 供「上傳總表」歷史匯入比對用，內部料號改名不受影響
...
def list_sku():
    ...
    def _build_q():
        ...
        if kw:
            q = q.or_(f'internal_code.ilike.%{kw}%,erp_code.ilike.%{kw}%,ean.ilike.%{kw}%,report_product_name.ilike.%{kw}%,report_short_name.ilike.%{kw}%,shipping_system_product_name.ilike.%{kw}%,shipping_system_product_id.ilike.%{kw}%,ledger_history_sku_name.ilike.%{kw}%')
        ...
        # 2026-08-27修正：sort_order有重複值（tie）時，加上id當次要排序鍵，
        # 確保排序結果每次查詢都固定一致，跟「對帳查詢→依內部料號名矩陣」
        # 匯出的排序邏輯用同一套tie-break，兩邊才不會對不起來。
...
def create_sku():
    ...
    rec = {
        ...
        'ean': data.get('ean'),
        # 2026-08-25新增：出貨系統「商品編號」，比對可靠度比商品名稱高（定位比照EAN，
        # 純輔助比對欄位，非業務主鍵）。
        'shipping_system_product_id': data.get('shipping_system_product_id'),
        ...
        # 2026-09-11新增：「上傳總表」(import_invoice_ledger_history()) 歷史匯入比對用的
        # 專用名稱，跟shipping_system_product_name同一套定位——內部料號本身改名時不受影響，
        # 留空則比對邏輯退回用internal_code本身（相容既有總表）。
        'ledger_history_sku_name': data.get('ledger_history_sku_name'),
...
def update_sku(sid):
    data = request.json or {}
    rec = {}
    for k in ('internal_code', 'erp_code', 'ean', 'shipping_system_product_id', 'shipping_system_product_name', 'ledger_history_sku_name', 'model_code', 'report_product_name', 'report_short_name',
    ...
    # 2026-08-31新增：內部料號文字若被改名（例如補上「6吋/7.8吋」、加「殼」等說明字樣使名稱
    # 更清楚），「經銷商品項對照表」(dealer_sku_mapping) 裡原本用舊文字設定好的主料號/贈品
    # 料號1~3，若不同步跟著改，就會變成「文字對不上」的孤兒設定——比對時抓不到、拆解驗證檔
    # 頁籤二（依財報料號品名呈現）也會抓不到財報料號品名而退回顯示料號本身。改名時一併把
    # 對照表裡用到舊文字的欄位同步更新成新文字，避免每次改名都要事後人工抓漏。
    old_code = None
    new_code = None
    if 'internal_code' in rec:
        old_row = sb.table('internal_sku_list').select('internal_code').eq('id', sid).execute()
        old_rows = old_row.data or []
        old_code = old_rows[0].get('internal_code') if old_rows else None
        new_code = _norm(rec.get('internal_code'))
        rec['internal_code'] = new_code

    # 2026-08-31新增：財報料號品名若被改名，「書店經銷商方案」明細表(bookstore_plan_items)
    # 用來當拆帳明細鍵值的report_product_name欄位，也是複製當下文字，改名不同步一樣會變成
    # 孤兒設定，跟上面internal_code是同一種問題。但report_product_name是「多筆內部料號共用
    # 同一品名」的多對一欄位（見「書店經銷商方案」拆帳明細區段說明），不像internal_code是
    # 1對1，不能無條件同步——如果改的這筆料號只是被「重新歸類」到別的品名（原品名底下還有
    # 其他料號在用），舊名稱其實還沒有失效，此時同步反而會把書店方案裡本來正確的舊名稱誤改
    # 掉。因此下面會先檢查「改名後，整個內部料號清單裡是否已經沒有任何一筆還在用這個舊
    # 品名」，確定是真正的「改名」而非「重新歸類」才同步。
    old_name = None
    new_name = None
    if 'report_product_name' in rec:
        old_name_row = sb.table('internal_sku_list').select('report_product_name').eq('id', sid).execute()
        old_name_rows = old_name_row.data or []
        old_name = old_name_rows[0].get('report_product_name') if old_name_rows else None
        new_name = _norm(rec.get('report_product_name'))
        rec['report_product_name'] = new_name

    sb.table('internal_sku_list').update(rec).eq('id', sid).execute()

    if old_code and new_code and old_code != new_code:
        for col in ('main_sku', 'gift_sku1', 'gift_sku2', 'gift_sku3'):
            sb.table('dealer_sku_mapping').update({col: new_code, **_audit_upd()}).eq(col, old_code).execute()
        # 書店方案明細的「子料號」欄位同樣是複製internal_code文字，內部料號本身是1對1，
        # 可以直接無條件同步（不像下面report_product_name需要額外檢查）。
        sb.table('bookstore_plan_items').update({'sub_internal_code': new_code}).eq('sub_internal_code', old_code).execute()

    if old_name and new_name and old_name != new_name:
        still_used = sb.table('internal_sku_list').select('id').eq('report_product_name', old_name).limit(1).execute()
        if not still_used.data:
            sb.table('bookstore_plan_items').update({'report_product_name': new_name}).eq('report_product_name', old_name).execute()
...
def delete_sku(sid):
    ...
    if code:
        ...
        if not ledger_hit.data:
            # 2026-09-02修正：一勞永逸方案——文字比對不到時，再用internal_code_id補查一次，
            # 避免這筆料號的文字快照在sku_invoice_ledger早就跟目前主檔文字不同步（例如中途
            # 改過名），單靠文字比對誤判成「沒有被使用」而放行刪除，造成已存好的id關聯跟著
            # 被刪除失去意義。
            ledger_hit = sb.table('sku_invoice_ledger').select('id').eq('internal_code_id', sid).limit(1).execute()
        if ledger_hit.data:
...
def sku_template():
    ...
    headers = ['料號序號', '內部料號', 'EAN', '出貨商品Id', '出貨系統商品名稱', '發票總表比對名稱', 'ERP順序', '料號', '財報料號品名', '財報簡稱', '產品定價', '商品進貨未稅價', '配件', '備註', '方案清單顯示', '發票總表顯示']
    required = {'料號序號'}
    notes_row = [
        _SKU_NOTE_ROW_MARKER,
        '僅供對照識別，此欄位異動不會被套用（如需修改內部料號請至畫面上編輯該筆料號）',
        '可留空，廠商條碼（留空不會覆蓋既有值）；上傳對帳檔比對時會用來輔助自動比對內部料號',
        '可留空，出貨系統「商品編號」（留空不會覆蓋既有值）；供未來「出庫數比對未開發票數」功能比對用，可靠度比商品名稱高',
        '可留空，出貨系統匯出檔的「商品名稱」欄位文字（留空不會覆蓋既有值）；供未來「出庫數比對未開發票數」功能比對用',
        '可留空（留空不會覆蓋既有值）；「上傳總表」比對用專屬名稱，設定後即使內部料號改名也不影響比對舊總表欄位，留空則退回用內部料號本身比對',
        '可留空（留空不會覆蓋既有值）',
        '可留空（留空不會覆蓋既有值）',
    ...
    widths = [14, 22, 16, 14, 22, 22, 12, 14, 26, 18, 14, 18, 10, 26, 14, 14]
    ...
    ws.append(['C0001', 'SAMPLE-DEMO-0001（此列為範例，請刪除後填入實際資料）', '4710000000000', '643', '範例出貨系統商品名稱', '', 'ERP-001', 'MODEL-X', '財報範例品名', '財報簡稱範例', 1990, 1200, '是', '', '顯示', '顯示'])
...
def add_template_sku():
    ...
    headers = ['內部料號', 'EAN', '出貨商品Id', '出貨系統商品名稱', '發票總表比對名稱', 'ERP順序', '料號', '財報料號品名', '財報簡稱', '產品定價', '商品進貨未稅價', '配件', '備註', '方案清單顯示', '發票總表顯示']
    required = {'內部料號'}
    notes_row = [
        '必填，不可與清單中既有內部料號重複；若重複，此列會被略過並列在匯入結果中，不會覆蓋既有資料',
        '可留空，廠商條碼；上傳對帳檔比對時會用來輔助自動比對內部料號',
        '可留空，出貨系統「商品編號」；供未來「出庫數比對未開發票數」功能比對用，可靠度比商品名稱高',
        '可留空，出貨系統匯出檔的「商品名稱」欄位文字；供未來「出庫數比對未開發票數」功能比對用',
        '可留空；「上傳總表」比對用專屬名稱，設定後即使內部料號改名也不影響比對舊總表欄位，留空則退回用內部料號本身比對',
        '可留空', '可留空',
    ...
    nfill = openpyxl.styles.PatternFill('solid', fgColor='F2F2F2')
    nfont = openpyxl.styles.Font(italic=True, color='888888')
    widths = [22, 16, 14, 22, 22, 12, 14, 26, 18, 14, 18, 10, 26, 14, 14]
    ...
    ws.append(['SAMPLE-DEMO-0001（此列為範例，請刪除後填入實際資料）', '4710000000000', '643', '範例出貨系統商品名稱', '', 'ERP-001', 'MODEL-X', '財報範例品名', '財報簡稱範例', 1990, 1200, '是', '', '顯示', '顯示'])
...
def export_sku_list():
    ...
    headers = ['排序', '料號序號', '內部料號', 'EAN', '出貨商品Id', '出貨系統商品名稱', '發票總表比對名稱', 'ERP順序', '料號', '財報料號品名', '財報簡稱', '產品定價', '商品進貨未稅價', '配件', '備註', '方案清單顯示', '發票總表顯示']
    ws.append(headers)
    for i, s in enumerate(all_rows, 1):
        ws.append([
            i, s.get('sku_seq_no'), s.get('internal_code'), s.get('ean'), s.get('shipping_system_product_id'), s.get('shipping_system_product_name'), s.get('ledger_history_sku_name'), s.get('erp_code'), s.get('model_code'),
...
def batch_modify_import_sku():
    ...
    _UPDATE_FIELDS = ['erp_code', 'ean', 'shipping_system_product_id', 'shipping_system_product_name', 'ledger_history_sku_name', 'model_code', 'report_product_name', 'report_short_name',
...
def add_batch_import_sku():
    ...
    for ri, row in enumerate(rows[1:], 2):
        ...
        rec = {
            ...
            'ean': _get(row, 'ean'),
            'shipping_system_product_id': _get(row, 'shipping_system_product_id'),
            'shipping_system_product_name': _get(row, 'shipping_system_product_name'),
            'ledger_history_sku_name': _get(row, 'ledger_history_sku_name'),
...
def _valid_sku_codes():
    """回傳目前「內部料號清單」所有有效的內部料號 -> id 對照字典，供經銷商品項對照表
    存檔前驗證用，同時也是2026-09-01改用id關聯後，把使用者輸入的料號文字解析成id的
    唯一入口。呼叫端原本只用`in`判斷是否存在（dict跟set的`in`行為相同，改回傳型別不影響
    既有呼叫端），新增呼叫端可以直接用.get(code)拿到對應id一併寫入*_id欄位。
    ...
    rows = _fetch_all_rows(lambda: sb.table('internal_sku_list').select('internal_code,id'))
    return {r['internal_code']: r['id'] for r in rows if r.get('internal_code')}


def _check_sku_refs(main_sku, gift_skus, valid_codes):
    """檢查內部料號/贈品料號是否都存在於內部料號清單，不存在則回傳錯誤訊息（None代表通過）"""
    if main_sku and main_sku not in valid_codes:
        return f'內部料號「{main_sku}」不存在於「內部料號清單」，請先在該清單新增此料號，或修正為既有料號'
    for gv in gift_skus:
...
def _sku_id_fields(main_sku, gift_sku1, gift_sku2, gift_sku3, code_to_id,
                    main_key='main_sku_id', gift1_key='gift_sku1_id',
                    gift2_key='gift_sku2_id', gift3_key='gift_sku3_id'):
    """2026-09-01新增：把「比對/選擇出來的料號文字」解析成內部料號清單的id（見v1.46
    schema新增的*_id欄位）。code_to_id用_valid_sku_codes()查出來的{internal_code:id}
    字典即可（呼叫端通常已經為了驗證而查過一次，直接重用，不用再查一次資料庫）。文字對到
    目前主檔就填對應id；文字本身是None、或文字已經是「查不到的孤兒文字」（極少數情況，
    例如手動比對時前端選單資料剛好跟後端查的當下有時間差），id就填None——不是錯誤，只是
    這筆之後仍會退回用文字比對（過渡期相容行為），不會擋下整筆操作。呼叫端可用不同的
    *_key參數，讓同一份小工具同時服務欄位命名不同的dealer_sku_mapping/
    dealer_reconcile_lines兩種資料表。"""
    return {
        main_key: code_to_id.get(main_sku) if main_sku else None,
        gift1_key: code_to_id.get(gift_sku1) if gift_sku1 else None,
        gift2_key: code_to_id.get(gift_sku2) if gift_sku2 else None,
        gift3_key: code_to_id.get(gift_sku3) if gift_sku3 else None,
    }
...
def create_mapping():
    ...
    if not rec['dealer_name'] or not rec['main_sku']:
        return jsonify({'error': '來源經銷商與內部料號必填'}), 400
    code_to_id = _valid_sku_codes()
    err = _check_sku_refs(rec['main_sku'],
                          [rec['gift_sku1'], rec['gift_sku2'], rec['gift_sku3']],
                          code_to_id)
    if err:
        return jsonify({'error': err}), 400
    # 2026-09-01新增：同時存下id（見v1.46 schema），之後比對引擎/反查邏輯優先用id，
    # 不受內部料號文字之後被改名影響。
    rec.update(_sku_id_fields(rec['main_sku'], rec['gift_sku1'], rec['gift_sku2'], rec['gift_sku3'],
                              code_to_id, 'main_sku_id', 'gift_sku1_id', 'gift_sku2_id', 'gift_sku3_id'))
    rec.update(_audit_new())
...
def update_mapping(mid):
    ...
    if any(k in rec for k in ('main_sku', 'gift_sku1', 'gift_sku2', 'gift_sku3')):
        ...
        merged_main = _norm(rec.get('main_sku', cur.get('main_sku')))
        merged_gifts = [rec.get(k, cur.get(k)) for k in ('gift_sku1', 'gift_sku2', 'gift_sku3')]
        code_to_id = _valid_sku_codes()
        err = _check_sku_refs(merged_main, merged_gifts, code_to_id)
        if err:
            return jsonify({'error': err}), 400
        # 2026-09-01新增：料號有異動時，同步重新解析id（見v1.46 schema），避免這筆對照
        # 表之後也變成孤兒。
        rec.update(_sku_id_fields(merged_main, merged_gifts[0], merged_gifts[1], merged_gifts[2],
                                  code_to_id, 'main_sku_id', 'gift_sku1_id', 'gift_sku2_id', 'gift_sku3_id'))
    rec.update(_audit_upd())
...
def mapping_template():
    ...
    headers = ['來源經銷商', '經銷商代碼', 'EAN', '商品名稱', '內部料號',
...
# ── 經銷商品項對照表 — 料號健檢（2026-08-31新增） ──
@dealer_bp.route('/api/dealer/mapping/health-check', methods=['GET'])
@dealer_view_required
def mapping_health_check():
    """掃描全部「經銷商品項對照表」(dealer_sku_mapping)，找出主料號/贈品料號1~3裡，文字
    對不到目前「內部料號清單」任何一筆內部料號的設定（多半是內部料號清單後來改名，但這裡
    當初設定的舊文字沒有跟著更新造成），並用difflib找出清單裡最相似的一筆當作修正建議，
    供使用者用「批次修改匯入」一次修正。背景：發現「拆解驗證檔」頁籤二依財報料號品名分組
    時，部分料號抓不到品名而退回顯示料號本身，根因就是這種對照表跟料號清單文字對不起來的
    孤兒設定；2026-08-31起 update_sku() 已改成改名時自動同步對照表，這裡是抓出「已經存在
    的舊孤兒設定」用的一次性健檢工具。

    2026-09-11修正：v1.46/v1.47已改用internal_sku_list.id當比對關聯依據取代文字比對（見
    project_sku_id_based_matching_migration），只要對應的_id欄位已經有值，畫面顯示/金額
    計算就都是正確的，文字欄位只是尚未同步的歷史快照，不是真孤兒。若不比對_id欄位，這個
    健檢會把「已經用id修好、只是文字快照沒跟著更新」的舊記錄跟「真的沒有id、需要人工處理」
    的新孤兒混在一起，造成永遠有一堆看似待辦但其實已經解決的假警訊。因此以下兩段皆改為
    「文字對不上，且對應_id欄位也是NULL，才真正算孤兒」——已經有id的略過不列入報告。"""
    import difflib
    sku_rows = _fetch_all_rows(lambda: sb.table('internal_sku_list').select('internal_code,report_product_name'))
    valid_codes = [r['internal_code'] for r in sku_rows if r.get('internal_code')]
    valid_set = set(valid_codes)

    map_rows = _fetch_all_rows(lambda: sb.table('dealer_sku_mapping').select('*').order('id'))

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = '料號健檢'
    ws.append(['經銷商品項對照表ID', '來源經銷商', '經銷商代碼', 'EAN', '商品名稱',
               '欄位', '目前設定的料號(對不到清單)', '建議修正為(清單裡最相似的料號)', '差異說明'])

    hit = 0
    for m in map_rows:
        for col, id_col, label in (
            ('main_sku', 'main_sku_id', '內部料號'), ('gift_sku1', 'gift_sku1_id', '贈品料號1'),
            ('gift_sku2', 'gift_sku2_id', '贈品料號2'), ('gift_sku3', 'gift_sku3_id', '贈品料號3'),
        ):
            code = m.get(col)
            if not code or code in valid_set:
                continue
            if m.get(id_col):
                # id已經正確對到內部料號清單，文字只是尚未同步的歷史快照，不算真孤兒
                continue
            hit += 1
            close = difflib.get_close_matches(code, valid_codes, n=1, cutoff=0.5)
            suggestion = close[0] if close else '(找不到相似料號，請人工確認)'
            diff_text = ''
            if close:
                sm = difflib.SequenceMatcher(a=code, b=suggestion)
                parts = []
                for tag, i1, i2, j1, j2 in sm.get_opcodes():
                    if tag == 'equal':
                        continue
                    a_seg, b_seg = code[i1:i2], suggestion[j1:j2]
                    if tag == 'replace':
                        parts.append(f'第{i1+1}~{i2}字「{a_seg}」vs建議「{b_seg}」不同')
                    elif tag == 'delete':
                        parts.append(f'目前多了「{a_seg}」')
                    elif tag == 'insert':
                        parts.append(f'目前少了「{b_seg}」')
                diff_text = '；'.join(parts)
            ws.append([m.get('id'), m.get('dealer_name'), m.get('dealer_code'), m.get('ean'),
                       m.get('product_name'), label, code, suggestion, diff_text])

    for ci, w in enumerate([16, 14, 12, 16, 24, 10, 26, 26, 40], 1):
        ws.column_dimensions[ws.cell(1, ci).column_letter].width = w
    if hit == 0:
        ws.append(['目前沒有發現對不到內部料號清單的設定，全部經銷商品項對照表資料都正常。'])

    # ── 書店經銷商方案明細(bookstore_plan_items)一併健檢（2026-08-31新增）──
    # 跟上面dealer_sku_mapping同一種孤兒設定風險：拆帳明細鍵值report_product_name、
    # 子料號sub_internal_code都是複製internal_sku_list當下文字，改名不同步就會變成孤兒
    # 設定。update_sku()已對兩者加上改名時自動同步（report_product_name有額外的「確定
    # 舊名稱真的沒人在用才同步」保護，見該處註解），這裡同樣是抓「已存在的舊孤兒設定」
    # 用的一次性健檢工具，跟上面dealer_sku_mapping那段共用同一份Excel、不同工作表。
    valid_report_names = {r['report_product_name'] for r in sku_rows if r.get('report_product_name')}
    valid_report_name_list = list(valid_report_names)
    plan_rows = _fetch_all_rows(lambda: sb.table('bookstore_plans').select('id,plan_code,plan_name'))
    plan_by_id = {p['id']: p for p in plan_rows}
    plan_item_rows = _fetch_all_rows(lambda: sb.table('bookstore_plan_items').select('*').order('id'))

    ws2 = wb.create_sheet('書店方案健檢')
    ws2.append(['方案項目ID', '方案編號', '方案中文名稱', '欄位', '目前設定(對不到清單)',
                '建議修正為(清單裡最相似的)', '差異說明'])
    hit2 = 0
    for it in plan_item_rows:
        plan = plan_by_id.get(it.get('plan_id'), {})
        for col, id_col, label, valid_this, candidates in (
            ('report_product_name', 'report_product_name_id', '財報料號品名', valid_report_names, valid_report_name_list),
            ('sub_internal_code', 'sub_internal_code_id', '子料號', valid_set, valid_codes),
        ):
            val = it.get(col)
            if not val or val in valid_this:
                continue
            if it.get(id_col):
                # id已經正確補上，文字只是尚未同步的歷史快照，不算真孤兒
                continue
            hit2 += 1
            close = difflib.get_close_matches(val, candidates, n=1, cutoff=0.5)
            suggestion = close[0] if close else '(找不到相似的，請人工確認)'
            diff_text = ''
            if close:
                sm = difflib.SequenceMatcher(a=val, b=suggestion)
                parts = []
                for tag, i1, i2, j1, j2 in sm.get_opcodes():
                    if tag == 'equal':
                        continue
                    a_seg, b_seg = val[i1:i2], suggestion[j1:j2]
                    if tag == 'replace':
                        parts.append(f'第{i1+1}~{i2}字「{a_seg}」vs建議「{b_seg}」不同')
                    elif tag == 'delete':
                        parts.append(f'目前多了「{a_seg}」')
                        ...
                        parts.append(f'目前少了「{b_seg}」')
                diff_text = '；'.join(parts)
            ws2.append([it.get('id'), plan.get('plan_code'), plan.get('plan_name'),
                        label, val, suggestion, diff_text])
    for ci, w in enumerate([12, 14, 24, 10, 26, 26, 40], 1):
        ws2.column_dimensions[ws2.cell(1, ci).column_letter].width = w
    if hit2 == 0:
        ws2.append(['目前沒有發現對不到內部料號清單/財報料號品名的書店方案設定，全部正常。'])

    buf = io.BytesIO()
    wb.save(buf)
    buf.seek(0)
    return send_file(buf, as_attachment=True, download_name='料號健檢_經銷商對照表與書店方案.xlsx',
                      mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
def batch_import_mapping():
    ...
    for ri, row in enumerate(rows[1:], 2):
        ...
        # 2026-09-01新增：同步存入id關聯欄位（一勞永逸方案——之後顯示一律以id反查中文，
        # 不再依賴這裡存的文字，料號改名也不會變孤兒）
        rec.update(_sku_id_fields(main_sku, gift_sku1, gift_sku2, gift_sku3, valid_sku_codes,
                                   'main_sku_id', 'gift_sku1_id', 'gift_sku2_id', 'gift_sku3_id'))
        rec.update(_audit_new())
        batch.append(rec)
...
def _parse_workbook(wb, sheets_config):
    ...
    for cfg in sheets_config or []:
        ...
        for row in ws.iter_rows(min_row=header_row + 1, values_only=True):
            ...
            # 2026-09-01新增：使用者要求——各經銷商對帳檔若當初只提供「含稅金額」（未稅
            # 金額欄位對照沒有設定，或該列儲存格剛好留空/0），拆到後續所有報表分析時一律
            # 要能看到未稅金額，不能讓未稅金額整筆變成0——用「含稅金額÷1.05」在解析來源
            # 這裡一次性換算好，下游（dealer_reconcile_lines/dealer_reconcile_summary/
            # commit_batch()/export_batch()/_compute_dealer_batch_summary_pivot()等）全部
            # 自動拿到正確的未稅金額，不用每個報表各自補一次退路（這正是2026-08-31「親子
            # 天下C0045全無金額」那次修正只在_compute_dealer_batch_summary_pivot()局部加
            # 退路、沒有從源頭換算的殘留問題——那次退路是直接把含稅金額當未稅金額用，沒有
            # 除以1.05，會把未稅單價高估5%）。反過來，只有未稅、沒有含稅時，同樣用×1.05
            # 換算補齊含稅金額。兩欄都是0才不轉換，避免真正金額就是0的列被誤判成缺欄位。
            if not amount_untaxed and amount_taxed:
                amount_untaxed = round(amount_taxed / 1.05, 2)
            elif not amount_taxed and amount_untaxed:
                amount_taxed = round(amount_untaxed * 1.05, 2)
...
def _next_recon_seq_no():
    ...
    方便使用者在「對帳查詢」矩陣、「上傳團體訂單含發票」列表對照是哪一次上傳/入庫。"""
...
def _ingest_reconcile_file(raw_bytes, filename, dealer_override, period, upload_group,
    ...
    團體訂單資訊勾選匯入」功能（reconcile_import_from_group_invoices()）共用，避免兩份幾乎
    ...
    if dealer_override == '書店':
        ...
        # 2026-08-27曾短暫擴大到墊腳石/親子天下（雙軌規則：出貨單只計數量、對帳檔只計
        # 金額），但使用者反饋這樣會強制每次都要記得一起上傳出貨單，忘記上傳就會漏算
        # 數量，風險較高，故先改回僅香港經銷商適用。_parse_shipping_order()解析邏輯
        # 本身已驗證對墊腳石/親子天下的出貨單格式也通用，未來如需重新啟用可直接復原
        # 這裡跟下方Excel分支的判斷式即可，不用重寫解析邏輯。
        if dealer_override == '香港經銷商':
    ...
    # 2026-09-01新增：一次查詢「內部料號」文字->id字典，供這批明細比對結果同步存id
    # （一勞永逸方案——之後顯示一律以id反查中文，不受料號改名影響）
    code_to_id = _valid_sku_codes()
    ...
    for ln in lines:
        ...
        rec = {
            ...
            'matched_main_sku_id': None,
            'matched_gift_sku1_id': None,
            'matched_gift_sku2_id': None,
            'matched_gift_sku3_id': None,
        ...
        if ln.get('_prematched'):
            ...
            # 2026-09-02修正：優先採用來源（如書店拆帳引擎）已解析好的matched_main_sku_id，
            # 避免對「財報料號品名」重複做文字比對而遺失id優先的保護
            rec['matched_main_sku_id'] = ln.get('matched_main_sku_id') or (code_to_id.get(rec['matched_main_sku']) if rec['matched_main_sku'] else None)
        ...
        if m:
            ...
            # 2026-09-02修正：優先直接採用經銷商品項對照表(dealer_sku_mapping)這筆對應
            # 「自己存的」main_sku_id/gift_sku1~3_id（該表在_match_one()回傳的m裡，
            # by_ean/by_code/by_name這3條路徑都是select('*')查出來的完整列，含id欄位；
            # 只有EAN內部料號清單直接比對的sku_by_ean備援路徑沒有id可用）。原本這裡不管
            # dealer_sku_mapping有沒有已經存好id，一律重新用code_to_id（目前料號清單文字
            # ->id字典）反查m['main_sku']這個文字快照——如果這筆對照表是很久以前建立、
            # 之後內部料號又被改名，m['main_sku']文字會變孤兒，即使dealer_sku_mapping.
            # main_sku_id當初其實有正確存到id，也會被這裡的文字反查失敗蓋成None，等於
            # 白白浪費了對照表自己存的id、讓新進來的訂單又變回孤兒。改成id優先：對照表
            # 有存id就直接沿用，沒有(舊資料/EAN備援路徑)才退回用文字反查一次。
            rec['matched_main_sku_id'] = m.get('main_sku_id') or code_to_id.get(rec['matched_main_sku'])
            rec['matched_gift_sku1_id'] = m.get('gift_sku1_id') or code_to_id.get(rec['matched_gift_sku1'])
            rec['matched_gift_sku2_id'] = m.get('gift_sku2_id') or code_to_id.get(rec['matched_gift_sku2'])
            rec['matched_gift_sku3_id'] = m.get('gift_sku3_id') or code_to_id.get(rec['matched_gift_sku3'])
...
# ── 2026-08-15新增：從「上傳團體訂單含發票」勾選匯入 ──────────────────────────────
# 背景：「上傳團體訂單含發票」（bookstore_group_invoices）透過排程自動收集發票PDF＋出貨單
...
def archive_reconcile_candidate(iid):
    """2026-08-21新增：「從『上傳團體訂單含發票』勾選匯入」候選清單的「已拆帳」欄——讓使用者
    ...
    for iid in invoice_ids:
        ...
        if not dealer_name:
            results.append({'invoice_id': iid, 'ok': False,
                             'error': '尚未設定「經銷商類型」，請先到「上傳團體訂單含發票」編輯這筆發票'})
            continue
...
def confirm_line(lid):
    ...
    # 2026-09-01新增：同步存id關聯欄位（一勞永逸方案）
    rec.update(_sku_id_fields(main_sku, rec['matched_gift_sku1'], rec['matched_gift_sku2'],
                               rec['matched_gift_sku3'], code_to_id,
                               'matched_main_sku_id', 'matched_gift_sku1_id',
                               'matched_gift_sku2_id', 'matched_gift_sku3_id'))
    ...
    if data.get('save_mapping') and dealer_name and not is_adjustment_row:
        ...
        map_rec.update(_sku_id_fields(main_sku, rec['matched_gift_sku1'], rec['matched_gift_sku2'],
                                       rec['matched_gift_sku3'], code_to_id,
                                       'main_sku_id', 'gift_sku1_id', 'gift_sku2_id', 'gift_sku3_id'))
...
def _price_maps_from_sku_rows(sku_rows):
    """依internal_sku_list查詢結果（至少需含id/internal_code/list_price 3個欄位）建立
    price_by_id/price_by_code查詢表（產品定價list_price）。定價空白或<=0視為「沒有定價」
    （例如「代銷商品」「電子書方案」這2個集合類品名，本來就不維護定價），回傳的map裡不會
    出現這種料號，供_split_line_amounts_by_price()判斷要不要參與比例分攤——2026-09-01新增，
    一勞永逸方案：一筆對帳明細比對到多個內部料號(主+贈品)時，依產品定價比例分攤金額，
    取代舊版「金額100%歸主料號、贈品固定0元」的寫死規則。"""
    price_by_id, price_by_code = {}, {}
    for r in sku_rows:
        p = _to_num_or_none(r.get('list_price'))
        if p and p > 0:
            if r.get('id'):
                price_by_id[r['id']] = p
            if r.get('internal_code'):
                price_by_code[r['internal_code']] = p
    return price_by_id, price_by_code


def _sku_price_maps():
    """獨立查詢internal_sku_list.list_price建立價格查詢表，供還沒有撈過sku_rows的呼叫端
    （如commit_batch()/export_batch()）使用；已經查過sku_rows的呼叫端（如
    _compute_dealer_batch_summary_pivot()）請直接呼叫_price_maps_from_sku_rows(sku_rows)
    重用同一次查詢結果，不用再多打一次DB。"""
    rows = _fetch_all_rows(lambda: sb.table('internal_sku_list').select('id,internal_code,list_price'))
    return _price_maps_from_sku_rows(rows)


def _split_line_amounts_by_price(matched, price_by_id, price_by_code,
                                  amount, amount_untaxed, amount_taxed, tax_amount):
    """2026-09-01新增，一勞永逸方案的核心分攤邏輯：一筆對帳明細比對到的所有內部料號
    （matched，[(內部料號文字, 內部料號id或None), ...]，包含主料號+最多3個贈品料號），
    依各自「產品定價」(list_price)佔「這些料號定價總和」的比例，把這筆明細的4個金額欄位
    （amount／amount_untaxed／amount_taxed／tax_amount）分攤到各自料號身上，取代舊版
    「金額100%歸主料號、贈品固定0元」的寫死規則。

    - 沒有定價（None/0，如「代銷商品」「電子書方案」這2個集合類品名，本來就不維護定價）的
      料號，分攤到的金額固定是0，不參與比例分配——跟改版前「贈品固定0元」的結果一致，只是
      現在「有定價的贈品」（如殼套、捲線器）能分到對應金額，不再一律歸零。
    - 若整筆比對到的料號全部都沒有定價（理論上少見，例如整筆只比對到代銷商品/電子書方案
      這種集合類品名，沒有任何實體主料號），全部金額退回算在第一筆（通常是主料號）身上，
      避免金額整筆憑空消失。
    - 每個金額欄位各自獨立分攤、各自處理捨入誤差：依比例算出的份額四捨五入到小數點後2位，
      最後一個「有定價」的料號改用「總額 - 前面已分出去的份額」回推，確保這筆明細所有比對
      到的料號分攤後加總，完全等於原始金額，不會因為逐筆四捨五入產生落差。
    - 數量(qty)不在這個函式處理——比對到的每個料號(主+贈品)一律認列這筆明細的完整數量，
      這個規則本來就是對的、使用者也沒有要求改變，維持在各呼叫端自己處理。
    """
    n = len(matched)
    results = [dict(amount=0, amount_untaxed=0, amount_taxed=0, tax_amount=0) for _ in range(n)]
    if n == 0:
        return results
    weights = []
    for code, sid in matched:
        price = price_by_id.get(sid) if sid else None
        if price is None and code:
            price = price_by_code.get(code)
        weights.append(price if price and price > 0 else 0)
    total_w = sum(weights)
    if total_w <= 0:
        results[0] = dict(amount=amount or 0, amount_untaxed=amount_untaxed or 0,
                           amount_taxed=amount_taxed or 0, tax_amount=tax_amount or 0)
        return results
    priced_idxs = [i for i, w in enumerate(weights) if w > 0]
    last_priced_idx = priced_idxs[-1]
    for field, total in (('amount', amount), ('amount_untaxed', amount_untaxed),
                         ('amount_taxed', amount_taxed), ('tax_amount', tax_amount)):
        total = total or 0
        running = 0
        for i in priced_idxs:
            if i == last_priced_idx:
                share = round(total - running, 2)
            else:
                share = round(total * weights[i] / total_w, 2)
                running += share
            results[i][field] = share
    return results
...
def commit_batch(bid):
    ...
    # 2026-09-01新增：一勞永逸方案——彙總改依「內部料號id」分組（而不是文字），料號改名
    # 造成的文字孤兒不再讓同一顆料號被拆成兩列，也不再因抓不到而整列消失。
    # code_to_id：目前「內部料號清單」文字->id（供舊資料只有文字、沒存id時，用目前主檔
    # 現況嘗試反查一次）；id_to_code：id->目前主檔文字（供決定彙總列要顯示的internal_code，
    # 一律用「現在」的文字，不用比對當下可能已經被改掉的舊文字）。
    code_to_id = _valid_sku_codes()
    id_to_code = {v: k for k, v in code_to_id.items()}
    price_by_id, price_by_code = _sku_price_maps()

    def _resolve_key(text, stored_id):
        """回傳(agg分組鍵, internal_sku_id, internal_code)。優先用寫入比對結果時就存好的id；
        沒有id欄位的舊資料，退回用「目前」的文字->id字典嘗試反查；兩者都對不到，才退回用
        文字本身當分組鍵（跟改版前行為一致，至少不會整列消失，只是仍會被當成孤兒無法對應
        到正確財報料號品名，需靠v1.46遷移SQL或手動指定id修正）。"""
        rid = stored_id or code_to_id.get(text)
        if rid:
            return ('id', rid), rid, id_to_code.get(rid, text)
        return ('code', text), None, text

    # 2026-09-01新增：一筆對帳明細比對到多個內部料號（1主料號+最多3贈品料號）時，金額不再
    # 100%歸主料號、贈品固定0元——改成依「產品定價」(list_price)比例分攤到所有比對到的料號，
    # 讓每個財報料號品名都能反映實際分攤到的金額。使用者已確認：「代銷商品」/「電子書方案」
    # 這2個名稱本來就不維護產品定價，分攤到的金額固定是0（維持現況），其餘有定價的料號才
    # 參與比例分攤，這正是_split_line_amounts_by_price()「沒有定價的料號分0元、只在有定價
    # 的料號之間依比例分」的預設行為。數量認列規則不變：每個比對到的料號(主+贈品)都認列
    # 這筆明細的完整數量，只有金額改用定價比例重新分配。
    agg = {}  # 分組鍵 -> {qty, amount, amount_taxed, amount_untaxed, tax_amount, internal_sku_id, internal_code}
    for l in lines:
        ...
        if main:
            matched.append((main, l.get('matched_main_sku_id')))
        for gift, gift_id in ((l.get('matched_gift_sku1'), l.get('matched_gift_sku1_id')),
                               (l.get('matched_gift_sku2'), l.get('matched_gift_sku2_id')),
                               (l.get('matched_gift_sku3'), l.get('matched_gift_sku3_id'))):
            if gift:
                matched.append((gift, gift_id))
        if not matched:
            continue

        shares = _split_line_amounts_by_price(matched, price_by_id, price_by_code,
                                               amount, amount_untaxed, amount_taxed, tax_amount)
        for (code, sid), share in zip(matched, shares):
            key, rid, resolved_code = _resolve_key(code, sid)
            a = agg.setdefault(key, {'qty': 0, 'amount': 0, 'amount_untaxed': 0, 'amount_taxed': 0,
                                      'tax_amount': 0, 'internal_sku_id': rid, 'internal_code': resolved_code})
            a['qty'] += qty  # 數量規則不變：主料號/贈品料號都認列這筆明細的完整數量
            a['amount'] += share['amount']
            a['amount_untaxed'] += share['amount_untaxed']
            a['amount_taxed'] += share['amount_taxed']
            a['tax_amount'] += share['tax_amount']
    ...
    for key, v in agg.items():
        summary_batch.append({
            'dealer_name': dealer_name, 'period': period, 'internal_code': v['internal_code'],
            'internal_sku_id': v['internal_sku_id'],
...
def export_batch(bid):
    ...
    lines = lines_res.data or []
    # 2026-08-31修正：加上.order('id')當次要排序鍵——sort_order有重複值(tie)，內部料號清單
    # 已超過1000筆會分頁抓取，若排序不穩定，同一頁邊界附近的資料在分頁時可能被跳過或重複，
    # 導致某些料號抓不到、name_by_code缺該料號→退回用內部料號本身當品名，跟畫面「內部料號
    # 清單」看到的財報料號品名對不起來（頁籤二看起來像沒分組）。做法比照list_sku()既有修正
    # （見2026-08-27修正comment）與_compute_bookstore_order_matrix()同類寫法，統一補上tie-break。
    sku_list = _fetch_all_rows(lambda: sb.table('internal_sku_list').select('*').order('sort_order').order('id'))

    wb = openpyxl.Workbook()
    ws1 = wb.active
    ws1.title = '原始明細'
    ws1.append(['類型', '經銷商代碼', 'EAN', '商品名稱', '數量', '單價', '金額', '未稅金額', '含稅金額', '稅額', '比對狀態',
                '內部料號', '贈品料號1', '贈品料號2', '贈品料號3'])
    for l in lines:
    ...
    # 2026-09-01修正：agg分組鍵改成「優先用比對時就存好的id，沒有id才退回用文字」——
    # 料號改名造成的文字孤兒，只要當初比對時有存到id，這裡就能正確併回同一組，不會因為
    # 文字被改過而拆成孤兒另外一列（下面name_by_id/name_by_code解析品名時同樣id優先）。
    # 2026-09-01新增：一筆對帳明細比對到多個內部料號（1主料號+最多3贈品料號）時，金額
    # 不再是「主料號全拿、贈品固定0元」，改用_split_line_amounts_by_price()依「產品定價」
    # 比例分攤，跟commit_batch()共用同一套邏輯，確保這個驗證用頁籤跟真正寫入
    # dealer_reconcile_summary的數字算法完全一致，不會各自維護一套算法又漂移。
    price_by_id, price_by_code = _sku_price_maps()
    agg = {}
    for l in lines:
        matched = []
        for code, code_id in (
                (l.get('matched_main_sku'), l.get('matched_main_sku_id')),
                (l.get('matched_gift_sku1'), l.get('matched_gift_sku1_id')),
                (l.get('matched_gift_sku2'), l.get('matched_gift_sku2_id')),
                (l.get('matched_gift_sku3'), l.get('matched_gift_sku3_id'))):
            if code:
                matched.append((code, code_id))
        if not matched:
            continue
        shares = _split_line_amounts_by_price(
            matched, price_by_id, price_by_code,
            l.get('amount') or 0, l.get('amount_untaxed') or 0,
            l.get('amount_taxed') or 0, l.get('tax_amount') or 0)
        for (code, code_id), share in zip(matched, shares):
            key = ('id', code_id) if code_id else ('code', code)
            a = agg.setdefault(key, {'qty': 0, 'amount': 0, 'amount_untaxed': 0, 'amount_taxed': 0, 'tax_amount': 0})
    ...
    # 2026-08-31修正：使用者反饋這個頁籤原本直接依「內部料號」逐列列出，但畫面/其他匯出
    # （如「產生總表分析(依經銷商)」）都是依「財報料號品名」呈現——多個內部料號可能共用
    # 同一個財報料號品名（如「6吋透明殼」跟其他色號都算「S002A019W-Mini透明殼」），若不
    # 先依財報料號品名分組，會看起來跟總表分析、對帳查詢等其他畫面的數字對不起來。改成：
    # 先查每個內部料號對應的財報料號品名，再依財報料號品名分組加總數量/金額；找不到財報
    # 料號品名設定的內部料號（理論上不會發生，因已由_check_sku_refs擋下匯入不存在料號）
    # 防禦性地用內部料號本身當品名，避免整列消失看不出差異。
    #
    # 2026-09-01修正（使用者實測回報，根因：內部料號改名/加發票狀態後綴造成的孤兒對照，
    # 見dealer_reconcile_summary.internal_code同類問題）：原本name_by_code/names_order是
    # 遍歷「整個內部料號清單」sku_list建立的，代表這個驗證用頁籤會把主檔裡完全沒出現在
    # 這次批次的料號也列出來（金額/數量皆0，只是不標黃）——這批實測檔案29種內部料號，
    # 匯出卻有105列，82列是無關的0元料號，雜訊多到很難一眼看出「這批到底哪些料號沒反查
    # 成功」。改成：sku_list只用來建立「料號→財報品名」「財報品名→(sort_order,id)排序鍵」
    # 兩個查詢表，names_order改成只收這次批次agg裡實際出現過的財報品名，並依sort_order/id
    # 排序（跟list_sku()/總表分析等既有頁面的排序慣例一致），找不到排序鍵的（理論上不會
    # 發生）防禦性排到最後面。
    # 2026-09-01修正：新增name_by_id（id->財報料號品名，一勞永逸方案的主要查詢表），
    # name_by_code保留給只有文字、沒存到id的舊資料當退路，兩者都查不到才用料號本身當品名。
    name_by_id = {}
    name_by_code = {}
    name_sort_key = {}
    for sku in sku_list:
        sid = sku.get('id')
        code = sku.get('internal_code')
        name = sku.get('report_product_name') or code or f'(id={sid})'
        if sid:
            name_by_id[sid] = name
        if code:
            # 2026-08-31修正：若同一內部料號重複出現(理論上不該發生，但料號清單無資料庫層級唯一
            # 約束、且分頁/排序問題已知曾造成資料異常)，優先保留「有設定財報料號品名」的那筆，
            # 不讓後面出現、report_product_name是空值而退回用料號本身當品名的重複列蓋掉前面已經
            # 抓到的正確品名，避免同一料號因抓取順序不同而忽正確忽錯誤。
            if code in name_by_code and name == code and name_by_code[code] != code:
                pass
            else:
                name_by_code[code] = name
        sk = (sku.get('sort_order') or 0, sid or 0)
        if name not in name_sort_key or sk < name_sort_key[name]:
            name_sort_key[name] = sk

    _FALLBACK_SORT_KEY = (999999999, 999999999)
    name_agg = {}
    for key, v in agg.items():
        kind, val = key
        name = name_by_id.get(val, val) if kind == 'id' else name_by_code.get(val, val)
        if name not in name_sort_key:
            name_sort_key[name] = _FALLBACK_SORT_KEY
        a = name_agg.setdefault(name, {'qty': 0, 'amount': 0, 'amount_untaxed': 0, 'amount_taxed': 0, 'tax_amount': 0})
        a['qty'] += v['qty']
        a['amount'] += v['amount']
        a['amount_untaxed'] += v['amount_untaxed']
        a['amount_taxed'] += v['amount_taxed']
        a['tax_amount'] += v['tax_amount']

    names_order = sorted(name_agg.keys(), key=lambda n: name_sort_key.get(n, _FALLBACK_SORT_KEY))

    ws2 = wb.create_sheet('依財報料號品名呈現')
    ws2.append(['財報料號品名', '數量', '金額', '未稅金額', '含稅金額', '稅額'])
    ...
    for name in names_order:
        v = dict(name_agg.get(name, {'qty': 0, 'amount': 0, 'amount_untaxed': 0, 'amount_taxed': 0, 'tax_amount': 0}))
        if name in _BOOKSTORE_ORDER_AGGREGATE_COL_NAMES:
            # 2026-09-01新增：代銷商品/電子書方案這2個集合類品名（沿用書店訂單拆帳模組
            # 既有規則，見_BOOKSTORE_ORDER_AGGREGATE_COL_NAMES），不管原始明細有多少筆
            # 比對到，這裡的「數量」固定顯示1（代表這是1份服務/方案，不是可累加的實體
            # 商品數量）；金額維持正常加總——這2個名稱本來就沒有維護「產品定價」，依上面
            # 新的定價比例分攤規則本來就分不到金額，固定是0，跟改版前的顯示結果一致。
            v['qty'] = 1
        row_idx = ws2.max_row + 1
        ws2.append([name, v['qty'], v['amount'], v['amount_untaxed'], v['amount_taxed'], v['tax_amount']])
...
# group_invoices.seller_name）全稱——批次若是透過「從上傳團體訂單含發票勾選匯入」產生，
...
# （見下方_attach_invoice_info()）；找不到對應發票的批次（例如手動上傳、非團體訂單資訊匯入的
...
#    這一列留著即可。
#
# 2026-08-31再修正：使用者反饋依「銷售單位」分組，會把「對帳查詢」畫面裡明明是不同拆帳編號
# （如C0005/C0007）的批次合併呈現成同一列，跟「對帳查詢」看到的拆帳編號對不起來、也不方便
# 逐筆核對。改成分組維度直接用「拆帳編號」(dealer_reconcile_batches.recon_seq_no)——每個
# 拆帳編號各自獨立條列呈現一個區塊，不同拆帳編號不再合併；銷售單位/經銷商名稱改成只當作
# 每個拆帳編號區塊的顯示標籤。「總表分析」跟「選取批次清單」(明細)都新增「拆帳編號」欄位，
# 方便使用者用同一個編號互相核對兩個頁籤。最下方「料號加總」也改成用SUM公式加總各拆帳編號
# 區塊自己的「數量」列儲存格，公式直接寫進儲存格，不再是Python預先算好的靜態數字。詳見下方
# _compute_dealer_batch_summary_pivot()/export_dealer_batch_summary_analysis()最新註解。
# ============================================================
def _attach_invoice_info(batches):
    """依batches(dealer_reconcile_batches列)裡的upload_group，反查bookstore_group_invoices.
    reconciled_upload_group拿到對應那筆團體訂單資訊的銷售單位/OA訂單編號/發票日期/發票號碼/
    價格(含稅)/出貨訂單編號，寫回每筆batch dict（key前綴inv_，找不到對應發票則維持None）。
    找不到對應發票很常見（手動上傳/非團體訂單資訊匯入的批次本來就沒有upload_group或對應不到），
    不算錯誤。"""
...
def _compute_dealer_batch_summary_pivot(batch_ids):
    """依所選批次(batch_ids，須已入庫)，算出「總表分析」（依拆帳編號分組）用的樞紐資料。
    回傳 None 代表所選批次裡沒有任何一筆是「已入庫」狀態，無法產生報表。

    2026-08-31修正：分組維度改用「拆帳編號」(dealer_reconcile_batches.recon_seq_no)——
    拆帳編號在_ingest_reconcile_file()裡是每次「上傳並比對」（同一upload_group，如香港
    經銷商固定出貨單+發票一起選取）就固定產生/沿用一個，理論上每一筆已入庫批次都會有值；
    仍保留防禦性fallback（極少數情況下recon_seq_no為空，例如更早期的舊資料）。銷售單位/
    經銷商名稱改成只當作每個拆帳編號區塊的顯示標籤，不再是實際分組鍵——同一經銷商分好幾次
    上傳、對帳查詢裡是不同拆帳編號的批次，這裡也會各自獨立成一個區塊，不會被合併成一列。
    """
    if not batch_ids:
        return None
    ...
    batches_res = sb.table('dealer_reconcile_batches') \
        .select('id,dealer_name,period,status,source_filename,upload_group,recon_seq_no,source_kind') \
    ...
    # 2026-08-31修正：分組鍵改用拆帳編號(recon_seq_no)；銷售單位(有對應團體訂單資訊)或經銷商
    # 名稱只作為該拆帳編號區塊的顯示標籤，同一拆帳編號下若出現不只一種標籤（理論上不會
    # 發生，只是保險），用「/」合併顯示、不遺漏任何一個。
    batch_seq = {}
    # 2026-09-01新增：batch_source_kind——記錄每個批次的source_kind('normal'／
    # 'invoice_amount_only'／'shipping_qty_only')，供下方彙總迴圈判斷這筆明細的數量/金額
    # 該不該計入（跟commit_batch()、backfill_multi_sku_allocation_20260901.py的
    # count_qty/count_amount判斷邏輯一致，見下方說明）。
    batch_source_kind = {}
    seq_names = []
    seq_label = {}
    for b in batches:
        seq = (b.get('recon_seq_no') or '').strip() or f'（無拆帳編號-批次{b["id"]}）'
        batch_seq[b['id']] = seq
        batch_source_kind[b['id']] = b.get('source_kind') or 'normal'
        if seq not in seq_names:
            seq_names.append(seq)
        label = b.get('inv_seller_name') or b.get('dealer_name') or '（未命名）'
        if seq not in seq_label:
            seq_label[seq] = label
        elif label not in seq_label[seq].split('/'):
            seq_label[seq] = f'{seq_label[seq]}/{label}'
    committed_ids = list(batch_seq.keys())

    lines = _fetch_all_rows(lambda: sb.table('dealer_reconcile_lines')
                             .select('batch_id,matched_main_sku,matched_gift_sku1,matched_gift_sku2,'
                                     'matched_gift_sku3,matched_main_sku_id,matched_gift_sku1_id,'
                                     'matched_gift_sku2_id,matched_gift_sku3_id,'
                                     'qty,amount_untaxed,amount_taxed,match_status')
    ...
    sku_rows = _fetch_all_rows(lambda: sb.table('internal_sku_list')
                                .select('id,internal_code,report_product_name,purchase_price_notax,'
                                        'list_price,sort_order,is_active').order('sort_order'))
    # 2026-09-01新增：name_by_id（id->財報料號品名）——一勞永逸方案的主要查詢表，讓這裡跟
    # export_batch()一樣改成id優先反查，內部料號改名不會再讓比對到的金額/數量從固定欄位
    # 的樞紐報表裡整筆消失（這是本次修法最早發現、最嚴重的一個bug：舊版找不到就直接用料號
    # 文字本身當name，一定對不上固定欄位columns，該筆資料就悄悄不見了）。
    name_by_id = {}
    name_by_code = {}
    cost_by_name = {}
    list_price_by_name = {}
    ...
    for s in sku_rows:
        ...
        sid = s.get('id')
        name = s.get('report_product_name') or code or (f'(id={sid})' if sid else None)
        if not name:
            continue
        if sid:
            name_by_id[sid] = name
        ...
        # 2026-09-02新增：list_price_by_name（財報料號品名->產品定價list_price，含稅）——
        # 供下方cell_by_group迴圈「數量>0但完全沒有比對到金額」時的補價估算使用，取值規則
        # 跟cost_by_name一致（同名底下多筆內部料號，取第一筆遇到的值當代表）。
        lp = _to_num_or_none(s.get('list_price'))
        if name not in list_price_by_name:
            list_price_by_name[name] = lp
    ...
    # 2026-08-21修正：同一拆帳編號底下所有明細直接加總數量與未稅金額，算出一個「加權平均
    # 單價」(=未稅金額加總/數量加總)，不再依單價分組，避免議價些微差異造成同一拆帳編號
    # 重複出現好幾次。
    # 2026-08-31修正：修正2個使用者回報的漏洞——
    # (a) amount_untaxed/amount_taxed擇一填：部分經銷商的原始檔案格式只會填其中一欄（依
    #     經銷商解析規則不同，例如含稅金額擇一填），原本只認amount_untaxed，該欄為0/None
    #     時金額整筆消失（親子天下C0045即此案例）——改成amount_untaxed為空時退回改用
    #     amount_taxed。**2026-09-01補充：這個退路只是暫時止血，沒有/1.05換算成真未稅，
    #     會把未稅單價高估5%——真正的修正已經搬到來源_parse_workbook()（見該函式2026-09-01
    #     新增的換算邏輯），從解析階段就把amount_untaxed/amount_taxed互相補齊，這裡的
    #     退路留著當最後一道防線（例如更早期、_parse_workbook()修正前就已經存進資料庫的
    #     舊資料）。**
    # (b) matched_gift_sku1/2/3（套組贈品比對結果）：原本只讀matched_main_sku，贈品料號
    #     比對結果完全沒被讀進來加總，導致該料號在總表分析裡數量整筆消失，或因為另一批次
    #     內同料號有以「主料號」身分出現、卻始終比對不到對應金額而顯示單價0元（萬耀科技
    #     C0044部分料號即此案例）——比照commit_batch()既有規則：贈品料號本身無獨立售價，
    #     只計入數量，金額固定為0（金額全部算在主料號那筆）。**2026-09-01改版：不再是
    #     「贈品固定0元」的硬規則，改成跟commit_batch()同一套_split_line_amounts_by_price()
    #     依產品定價比例分攤——沒有定價的料號（含代銷商品/電子書方案）依然分到0元，跟
    #     改版前結果一致，但真的有定價的贈品（如殼套、捲線器）現在能分到對應金額，不再
    #     固定歸零。**
    price_by_id, price_by_code = _price_maps_from_sku_rows(sku_rows)
    agg = {}  # (拆帳編號, report_product_name) -> {'qty':.., 'amount':..}
    for l in lines:
        if l.get('match_status') == 'unmatched':
            continue
        seq = batch_seq.get(l.get('batch_id'))
        if not seq:
            continue
        # 2026-09-01修正（Stacy回報：「單位團體(知識)」類經銷商發票+出貨單分開上傳、合併
        # 同一拆帳編號後，總表分析金額欄位整欄變0）：舊版不分批次來源一律「qty=0就整列
        # 跳過」，對「發票只計金額」(source_kind='invoice_amount_only')的批次是錯的——這類
        # 發票PDF本來就常常沒有逐列數量欄（數量另由出貨單提供，見_ingest_reconcile_file()
        # 說明），原始qty欄位是0/None，被這個filter直接整列跳過，連金額也一起漏算。改成
        # 比照commit_batch()/backfill_multi_sku_allocation_20260901.py既有的count_qty/
        # count_amount判斷：'invoice_amount_only'批次只計金額、不計數量（避免跟出貨單數量
        # 重複加總）；'shipping_qty_only'批次只計數量、不計金額（該批次金額本來就是0）；
        # 'normal'批次數量金額都計入。只有「兩者都算出0」才整列跳過，不再只憑原始qty欄位
        # 判斷是否要略過金額。
        source_kind = batch_source_kind.get(l.get('batch_id')) or 'normal'
        count_qty = source_kind != 'invoice_amount_only'
        count_amount = source_kind != 'shipping_qty_only'
        qty = (l.get('qty') or 0) if count_qty else 0
        amount_untaxed_in = (l.get('amount_untaxed') or 0) if count_amount else 0
        amount_taxed_in = (l.get('amount_taxed') or 0) if count_amount else 0
        if not qty and not amount_untaxed_in and not amount_taxed_in:
            continue
        matched = []
        main = l.get('matched_main_sku')
        if main:
            matched.append((main, l.get('matched_main_sku_id')))
        for gift, gift_id in ((l.get('matched_gift_sku1'), l.get('matched_gift_sku1_id')),
                               (l.get('matched_gift_sku2'), l.get('matched_gift_sku2_id')),
        ...
        shares = _split_line_amounts_by_price(
            matched, price_by_id, price_by_code,
            0, amount_untaxed_in, amount_taxed_in, 0)
        for (code, code_id), share in zip(matched, shares):
            name = name_by_id.get(code_id) or name_by_code.get(code) or code
            a = agg.setdefault((seq, name), {'qty': 0, 'amount': 0})
            a['qty'] += qty
            # 2026-09-01修正（Stacy回報「總表分析(依經銷商)」小計(未稅)把含稅金額當未稅
            # 顯示）：這裡原本是share['amount_untaxed'] or share['amount_taxed']——當來源
            # 明細的amount_untaxed是0/None（尚未跑backfill_multi_sku_allocation_20260901.py
            # 回填的舊資料，或該經銷商解析規則只填了含稅欄位）時，會直接把含稅金額當成未稅
            # 金額累加，「小計(未稅)」標籤名稱跟實際內容不一致，容易誤導使用者。改成：未稅
            # 為0但含稅有值時，用含稅÷1.05換算回真未稅再累加，跟_parse_workbook()/
            # backfill腳本Step A用的同一套換算規則一致，即使舊資料還沒回填，這裡顯示的
            # 「未稅」也名副其實；等舊資料回填完成後，amount_untaxed本身就不會是0，這段
            # 換算就不會被觸發，兩種情況殊途同歸算出同一個數字。
            untaxed_share = share['amount_untaxed']
            if not untaxed_share and share['amount_taxed']:
                untaxed_share = round(share['amount_taxed'] / 1.05, 2)
            a['amount'] += (untaxed_share or 0)

    # 2026-09-02第二次修正（Stacy回報：上一版每個機器/配件各自獨立用產品定價估算金額，
    # 直接「加」在集合類品名(電子書方案/代銷商品)原本的發票金額之上，導致小計(未稅)比實際
    # 匯入的發票金額還大很多（本例369,980.8遠大於實際發票190,476）——正確做法是「均攤」不是
    # 「疊加」：整個拆帳編號底下唯一的真金額來源，是集合類品名累加到的那筆發票金額（這裡
    # 叫它pool，也就是Stacy說的「最大筆」），要把pool依「產品定價(未稅)×數量」的比例，
    # 分攤到有數量但沒有比對到金額的機器/配件身上，分攤剩下的餘額才留在集合類品名自己身上，
    # 這樣「集合類品名剩餘金額 + 所有機器/配件分攤到的金額」加總後，永遠精確等於pool本身
    # （最後用減法算remainder，不是各自獨立四捨五入再加總，才不會有累積誤差）。
    # 只有在這個拆帳編號完全沒有pool可以均攤時（例如這個拆帳編號本身就沒有任何發票來源），
    # 才退回用「各自產品定價×數量」獨立估算——這種情況本來就沒有真實金額可以比較、不會有
    # 「小計超過實際匯入金額」的問題。
    cell_by_group = {name: {seq: None for seq in seq_names} for name in columns}
    missing_price_names = set()
    for seq in seq_names:
        # 1) 找這個拆帳編號底下集合類品名(電子書方案/代銷商品)目前累加到的金額——這是
        #    Stacy說的「最大筆」，也是這個拆帳編號唯一真正比對到的發票金額（pool）。
        pool_by_agg_name = {}
        for agg_name in _BOOKSTORE_ORDER_AGGREGATE_COL_NAMES:
            a = agg.get((seq, agg_name))
            if a and a['amount']:
                pool_by_agg_name[agg_name] = a['amount']
        total_pool = sum(pool_by_agg_name.values())

        # 2) 找這個拆帳編號底下「有數量、但完全沒有比對到金額」的一般品名（機器/配件）——
        #    有維護產品定價的納入weighted（準備依比例均攤pool），沒有維護產品定價的納入
        #    no_price（估不出金額，維持顯示0並列入警示）。
        weighted = []   # (name, qty, weight)　weight=產品定價(未稅)×數量
        no_price = []   # (name, qty)
        for name in columns:
            if name in _BOOKSTORE_ORDER_AGGREGATE_COL_NAMES:
                continue
            a = agg.get((seq, name))
            if not a or not a['qty'] or a['amount']:
                continue
            lp = list_price_by_name.get(name)
            if lp:
                weighted.append((name, a['qty'], (lp / 1.05) * a['qty']))
            else:
                no_price.append((name, a['qty']))

        total_weight = sum(w for _, _, w in weighted)
        allocated_total = 0
        if weighted and total_weight > 0 and total_pool > 0:
            # 有pool可以均攤：依權重比例把pool分給每個機器/配件，估算金額標記estimated=True，
            # below_cost仍用分攤後的未稅單價跟進貨未稅價比較。
            for name, qty, weight in weighted:
                amt = round(total_pool * weight / total_weight, 2)
                allocated_total += amt
                price = round(amt / qty, 2)
                cost = cost_by_name.get(name)
                cell_by_group[name][seq] = {
                    'price': price, 'qty': qty,
                    'below_cost': (cost is not None and price < cost),
                    'estimated': True,
                }
            remainder = round(total_pool - allocated_total, 2)
        elif weighted:
            # 這個拆帳編號完全沒有pool可以均攤（集合類品名沒有累加到任何金額）——退回用
            # 各自的產品定價×數量獨立估算，不會有「疊加超過實際匯入金額」的問題（因為本來
            # 就沒有pool金額可以比較）。
            for name, qty, weight in weighted:
                amt = round(weight, 2)
                price = round(amt / qty, 2)
        ...
        # 3) 連產品定價都沒有維護、估不出金額的機器/配件——保留數量、單價顯示0，記錄到
        #    missing_price_names，匯出時另開警示頁籤列出。
        for name, qty in no_price:
            cell_by_group[name][seq] = {'price': 0, 'qty': qty, 'below_cost': False, 'estimated': False}
            missing_price_names.add(name)

        # 4) 集合類品名本身：顯示均攤剩餘的金額（不是原始pool），數量固定顯示1，below_cost
        #    依設計不做紅字檢核。理論上一個拆帳編號底下只會有一個集合類品名有金額，若萬一
        #    同時有兩個（極少見），依各自原始金額比例分回剩餘，確保加總還是精確等於pool。
        if len(pool_by_agg_name) == 1:
            agg_name = next(iter(pool_by_agg_name))
            cell_by_group[agg_name][seq] = {'price': remainder, 'qty': 1, 'below_cost': False, 'estimated': False}
        elif len(pool_by_agg_name) > 1:
            for agg_name, orig in pool_by_agg_name.items():
                share = round(remainder * orig / total_pool, 2) if total_pool else 0
                cell_by_group[agg_name][seq] = {'price': share, 'qty': 1, 'below_cost': False, 'estimated': False}

        # 5) 一般品名本身已經直接比對到金額的情況（非集合類、qty>0且amount>0）——維持原本
        #    「加權平均未稅單價」邏輯，不受這次均攤修正影響。
        for name in columns:
            if name in _BOOKSTORE_ORDER_AGGREGATE_COL_NAMES:
                continue
            a = agg.get((seq, name))
            if a and a['qty'] and a['amount']:
                cost = cost_by_name.get(name)
    ...
    for name in columns:
        col_combined_total[name] = sum(
            cell_by_group[name][seq]['qty'] for seq in seq_names if cell_by_group[name][seq])
    ...
    return {
        ...
        'missing_price_names': sorted(missing_price_names),
...
def export_dealer_batch_summary_analysis():
    ...
    2026-08-31修正：依使用者需求全面改版——
    1) 樞紐分組維度改依「拆帳編號」(recon_seq_no)，每個拆帳編號各自獨立條列呈現（區塊間
       插入標題列＋淡色底），不再把不同拆帳編號的批次合併進同一列。
    2) 「選取批次清單」(明細)跟「總表分析」(總表)都新增「拆帳編號」欄位，方便使用者用同一個
       編號互相核對總表跟明細是否一致。
    3) 最下方「料號加總」改成用SUM公式加總每個拆帳編號區塊自己的「數量」列儲存格，而不是
       預先在Python算好的靜態數字——公式直接寫進儲存格，使用者在Excel裡就能看到公式來源、
       自行核對。
    """
    ...
    seq_names = pivot['seq_names']
    seq_label = pivot['seq_label']
    ...
    ws0 = wb.active
    ws0.title = '選取批次清單'
    # 2026-08-21新增：額外從對應的「上傳團體訂單含發票」帶出銷售單位/OA訂單編號/發票日期/發票號碼/
    # 價格(含稅)/出貨訂單編號（見_attach_invoice_info()）；批次若不是從團體訂單資訊匯入產生
    # （例如手動上傳），這幾欄留空。
    # 2026-08-31新增：「拆帳編號」欄位緊接在批次id後面，方便跟「總表分析」頁籤的拆帳編號
    # 互相核對（同一個編號在兩個頁籤都查得到，來源批次一目了然）。
    ws0.append(['批次id', '拆帳編號', '經銷商', '期間', '來源檔名', '銷售單位', 'OA訂單編號',
                '發票日期', '發票號碼', '價格(含稅)', '出貨訂單編號'])
    for b in pivot['batches']:
        ws0.append([b['id'], b.get('recon_seq_no'), b.get('dealer_name'), b.get('period'),
                    b.get('source_filename'), b.get('inv_seller_name'), b.get('inv_order_no'),
                    b.get('inv_invoice_date'), b.get('inv_invoice_no'), b.get('inv_amount_incl'),
                    b.get('inv_shipping_order_no')])
    for ci, w in enumerate([10, 12, 20, 12, 30, 20, 16, 12, 16, 12, 16], 1):
    ...
    # 2026-09-02新增：淡黃色底，標示這個儲存格的金額是用「內部料號清單」產品定價(含稅)/1.05
    # 回推估算出來的（數量>0但完全沒有比對到金額時的補價），不是實際比對到的交易金額。
    estfill = openpyxl.styles.PatternFill('solid', fgColor='FFF2CC')
    ...
    # 2026-08-31修正：A欄改成「拆帳編號」，B欄「銷售單位/經銷商」，C欄「項目」——比原本多
    # 一欄，讓拆帳編號可以直接當成篩選/核對用的欄位，不用只靠肉眼比對區塊標題列。
    BASE = 3  # A:拆帳編號, B:銷售單位/經銷商, C:列類型
    data_start = BASE + 1
    total_col2 = data_start + len(columns)

    ws4.cell(1, 1, '總表分析（依拆帳編號獨立分組；同一拆帳編號底下所有明細合併加總，單價為'
                    '加權平均未稅單價；每個拆帳編號各自成一個區塊，不與其他拆帳編號合併呈現；'
                    '淡黃色底的儲存格代表數量>0但沒有比對到金額，改用「內部料號清單」產品'
                    '定價(含稅)/1.05回推估算，非實際比對到的交易金額，請自行核對）')
    ...
    ws4.cell(3, 1, '拆帳編號').fill = hfill
    ws4.cell(3, 1).font = hfont
    ws4.cell(3, 1).alignment = wrap
    ws4.cell(3, 2, '銷售單位/經銷商').fill = hfill
    ws4.cell(3, 2).font = hfont
    ws4.cell(3, 2).alignment = wrap
    ws4.cell(3, 3, '項目').fill = hfill
    ws4.cell(3, 3).font = hfont
    ws4.cell(3, 3).alignment = wrap
    for ci, name in enumerate(columns, data_start):
    ...
    # 2026-08-31修正：依拆帳編號逐一獨立條列——每個拆帳編號接著「單價」「數量」2列明細，
    # 區塊之間不會互相合併，也不會被誤認為同一筆資料重複列出。同時記錄每個拆帳編號「單價」
    # /「數量」列的實際列號，供最下方「料號加總」寫公式使用（直接加總/相乘這些儲存格，
    # 不是預先在Python算好塞值）。第二輪修正：拆帳編號/銷售單位改成合併儲存格呈現（見下方
    # 迴圈內註解），不再額外插入整列的區塊標題列。
    ...
    for seq in seq_names:
        ...
        # 2026-08-31第二輪修正：使用者提供參考檔（如D2026SB0001銷售明細表）呈現方式是
        # 「拆帳編號」「銷售單位/經銷商」各自合併儲存格（跨單價+數量2列各顯示1次），不再
        # 額外插入一整列的區塊標題列（原本的「拆帳編號：Cxxxx｜xxx」整列，使用者確認不需要，
        # A/B欄本身已經看得到拆帳編號跟銷售單位/經銷商，不需要再重複一次）。
        r_price = r
        ws4.cell(r, 1, seq)
        ws4.cell(r, 2, label)
        ws4.cell(r, 3, '單價(未稅，加權平均)')
        for ci, name in enumerate(columns, data_start):
            cell = pivot['cell_by_group'][name][seq]
            if cell:
                c = _set_int_cell(ws4, r, ci, cell['price'])
                if cell.get('below_cost'):
                    c.fill = redfill
                    c.font = redfont
                elif cell.get('estimated'):
                    c.fill = estfill
        r += 1
        r_qty = r
        ws4.cell(r, 3, '數量')
        ...
        # 拆帳編號(A)/銷售單位/經銷商(B)各自合併「單價」「數量」這2列成一格，只保留左上角
        # (r_price列)的值，跟參考檔呈現方式一致。
        ws4.merge_cells(start_row=r_price, start_column=1, end_row=r_qty, end_column=1)
        ws4.merge_cells(start_row=r_price, start_column=2, end_row=r_qty, end_column=2)
        ws4.cell(r_price, 1).alignment = wrap
        ws4.cell(r_price, 2).alignment = wrap

        qty_rows.append(r_qty)
        price_rows.append(r_price)
    ...
    ws4.cell(r, 3, '料號加總(所選拆帳編號合計)')
    # 2026-08-31修正：改成公式直接加總每個拆帳編號區塊自己的儲存格，公式本身寫進儲存格
    # （不是Python算好的靜態數字），使用者在Excel裡打開就能看到公式來源、直接核對。
    # 2026-08-31第二輪修正：[代銷商品]/[電子書方案]這2個集合類品名（見
    # _BOOKSTORE_ORDER_AGGREGATE_COL_NAMES）的「數量」欄含意特殊（是方案組合數，不是可以
    # 直接加總的銷售數量），這2欄改成「單價×數量」逐項相乘再全部加總（等同該欄真正的金額
    # 加總），其餘一般品名欄維持原本直接加總「數量」列的SUM公式。
    for ci, name in enumerate(columns, data_start):
        col_letter = ws4.cell(1, ci).column_letter
        if name in _BOOKSTORE_ORDER_AGGREGATE_COL_NAMES:
            formula = '=' + '+'.join(
                f'{col_letter}{pr}*{col_letter}{qr}' for pr, qr in zip(price_rows, qty_rows))
        else:
            formula = '=SUM(' + ','.join(f'{col_letter}{qr}' for qr in qty_rows) + ')'
        _set_int_cell(ws4, r, ci, formula)
    ...
    # 2026-09-02新增：數量>0、完全沒有比對到金額、且「內部料號清單」也沒有維護產品定價，
    # 導致無法估算金額、單價只能顯示0的品名，另開一個警示頁籤列出，提醒使用者去補產品定價，
    # 而不是讓這種情況安靜地繼續顯示0元、卻沒有任何提示。
    if pivot.get('missing_price_names'):
        ws6 = wb.create_sheet('產品定價缺失警示')
        ws6.append(['財報料號品名（有數量但完全沒有比對到金額，且「內部料號清單」未維護產品定價，'
                     '無法估算金額，單價顯示為0，請補上產品定價）'])
        for name in pivot['missing_price_names']:
            ws6.append([name])
    ...
    buf.seek(0)
    if len(seq_names) == 1:
        fname = f"{seq_names[0]}_{seq_label.get(seq_names[0], '')}_總表分析.xlsx"
    else:
        fname = f"總表分析_{len(seq_names)}筆拆帳編號.xlsx"
    return send_file(buf, as_attachment=True, download_name=fname,
                      mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
def save_batch_notes(bid):
    ...
    填寫一次，入庫後無法修改；使用者反饋「從上傳團體訂單含發票勾選匯入」時，若來源發票的
    ...
    if 'period' in data:
        ...
        rec['period'] = p
    for k in ('reconcile_range', 'note', 'customer_type', 'reconcile_order', 'order_no', 'reconcile_no', 'bookstore_reconcile_no'):
        if k in data:
...
def _build_summary_data(dealer, period, code_kw, view, all_sku=False):
    ...
    # 2026-08-27修正：sort_order tie-break加id，比照list_sku()同一套排序規則
    sku_rows = _fetch_all_rows(lambda: sb.table('internal_sku_list').select('*').order('sort_order').order('id'))
    sku_map = {s['internal_code']: s for s in sku_rows}
    # 2026-09-01新增：一勞永逸方案——sku_by_id供依internal_sku_id優先反查現況資料，內部
    # 料號改名/加發票狀態後綴後，舊彙總列存的internal_code文字即使變成孤兒也能靠id正確
    # 反查到目前的內部料號/財報料號品名，不必再手動修正歷史資料才看得到正確數字。
    sku_by_id = {s['id']: s for s in sku_rows if s.get('id') is not None}

    def _resolve_summary_sku(r):
        """回傳(目前有效的內部料號文字, 對應內部料號清單row dict)。優先用彙總列存的
        internal_sku_id反查目前主檔現況；沒有id（尚未回填/尚未commit過的舊資料）才退回
        用internal_code文字比對（跟改版前行為一致）。"""
        sid = r.get('internal_sku_id')
        if sid is not None and sid in sku_by_id:
            s = sku_by_id[sid]
            return s.get('internal_code') or r['internal_code'], s
        return r['internal_code'], sku_map.get(r['internal_code'], {})

    if code_kw:
        # 2026-09-01修正：關鍵字比對也改用id優先反查到的「目前」內部料號文字，避免舊彙總列
        # 存的孤兒文字（料號已改名）用目前的料號關鍵字搜尋不到。
        rows = [r for r in rows if code_kw in _resolve_summary_sku(r)[0] or code_kw in r['internal_code']]
    ...
    if view == 'matrix':
        ...
        if all_sku:
            ...
            for code, sku in sku_map.items():
                columns.append({
                    ...
                    'id': sku.get('id') if sku.get('id') is not None else 0,  # 2026-08-27新增：tie-break見下方排序說明
                })
        else:
            for code in codes_seen:
                ...
                columns.append({
                    ...
                    'erp_code': sku.get('erp_code'),
                    'sort_order': sku.get('sort_order') if sku.get('sort_order') is not None else 999999,
                    'id': sku.get('id') if sku.get('id') is not None else 0,
                })
        # 2026-08-27修正：sort_order（內部料號清單「排序」欄位）在既有資料裡有不少重複值
        # （tie），原本只用sort_order排序時，Python的stable sort會用「這個內部料號在columns
        # 清單裡第一次被加進來的順序」當隱性的tie-break——而這個順序來自「這次查詢範圍內
        # 有銷售的料號」（依summary資料表回傳順序，跟sort_order無關）或sku_map.items()的
        # 插入順序，兩者都不保證跟「內部料號清單」畫面本身的排序（sort_order+id）一致，
        # 這正是使用者回報「橫軸料號品名順序跟內部料號清單排序對不起來」的根因。改成明確用
        # (sort_order, id)這組跟list_sku()完全相同的tie-break排序，才能保證兩邊順序一致。
        columns.sort(key=lambda c: (c['sort_order'], c['id']))
        ...
        for gkey, qmap in sorted(group_qty.items(),
            ...
            dealers_out.append({
                ...
                'order_no': rep.get('order_no') or '',  # 2026-08-27新增：訂單編號
    ...
    if view == 'matrix_report':
        ...
        col_info = {}        # col_key -> {'report_product_name','erp_code','sort_order','id'}
        ...
        def _upsert_col(ckey, sku, code):
            ...
            sid = sku.get('id') if sku.get('id') is not None else 0
            # 2026-08-27修正：原本只比較sort_order，sort_order有重複值（tie）時，同一個
            # col_key先被哪一筆sku upsert到就固定用那一筆的sort_order，造成「同分」的欄位
            # 之間彼此的先後順序取決於_upsert_col的呼叫順序（來自這次查詢範圍內剛好有銷售的
            # 料號，順序跟sort_order無關），跟「內部料號清單」畫面本身的排序（sort_order+id）
            # 對不起來——這正是使用者回報「橫軸料號品名順序從某欄開始完全跟內部料號清單
            # 對不起來」的根因。改成用(sort_order, id)tuple比較，並把id也存進col_info，
            # 讓下面columns排序時能用跟list_sku()完全相同的tie-break規則。
            if ckey not in col_info or (so, sid) < (col_info[ckey]['sort_order'], col_info[ckey]['id']):
                col_info[ckey] = {
        ...
        for r in rows:
            code, sku = _resolve_summary_sku(r)
            ckey = _col_key(code, sku)
            bid = r.get('batch_id')
            gkey = batch_to_group.get(bid, f'b{bid}')
            d = group_qty.setdefault(gkey, {})
        ...
        columns = sorted(col_info.keys(), key=lambda k: (col_info[k]['sort_order'], col_info[k]['id']))
    ...
    if view == 'report':
        ...
        for r in rows:
            code, sku = _resolve_summary_sku(r)
            key = sku.get('report_product_name') or code
...
def export_summary():
    ...
    if view == 'matrix':
        ...
        # 2026-08-27修正：原本head_cols漏掉「拆帳編號」欄（畫面thead第一欄就有，
        # 見renderQueryMatrix()，_build_summary_data()的dealers_out也早就有回傳
        # recon_seq_no，只是匯出這裡沒接上），且欄位順序跟畫面thead對不起來——
        # 改成跟畫面thead完全一致的順序：拆帳編號/經銷商/來源檔名/對帳順序/訂單編號/
        # 對帳編號/書店對帳編號/客戶類型/對帳月份/對帳區間/對帳備註。
        head_cols = ['拆帳編號', '經銷商', '來源檔名', '對帳順序', '訂單編號', '對帳編號', '書店對帳編號', '客戶類型', '對帳月份', '對帳區間', '對帳備註']
        ...
        for d in dealers:
            ...
            for i, c in enumerate(columns):
                ...
                row_total += q
                col_totals[i] += q
            ws.append([d.get('recon_seq_no') or '', d.get('dealer_name'), d.get('source_filename') or '', d.get('reconcile_order') or '',
                       d.get('order_no') or '', d.get('reconcile_no') or '', d.get('bookstore_reconcile_no') or '',
                       d.get('customer_type') or '', d.get('period'),
        ...
        columns = data.get('columns') or []
        dealers = data.get('dealers') or []
        # 2026-08-27修正：同上方view=='matrix'——補回畫面thead本來就有、匯出漏掉的
        # 「拆帳編號」欄（見renderQueryReportMatrix()），並改成跟畫面thead一致的欄位順序。
        head_cols = ['拆帳編號', '經銷商', '來源檔名', '對帳順序', '訂單編號', '對帳編號', '書店對帳編號', '客戶類型', '對帳月份', '對帳區間', '對帳備註']
        ...
        # 2026-08-31修正：Stacy要求「小計欄位含公式讓我驗證」——總計列的未稅/含稅金額小計
        # 原本是Python端算好的靜態數字，使用者無法在Excel點開這一格看到怎麼算出來的、
        # 沒辦法像SUMPRODUCT那樣肉眼核對是否跟明細加總一致（比照書店經銷商拆帳模組
        # SUMPRODUCT對不上金額合計那次修正的同一個訴求）。改成寫入=SUM(...)公式，
        # 讓使用者點開這一格能直接看到公式、驗算是否跟上方每一列的小計加總一致。
        # 資料列從第3列開始（第1列ERP順序、第2列內部料號），共len(dealers)列，
        # 總計列本身就是接下來append的這一列。
        first_data_row = 3
        last_data_row = 2 + len(dealers)
        total_row_num = last_data_row + 1
        untaxed_col_letter = openpyxl.utils.get_column_letter(len(head_cols) + len(columns) + 1)
        taxed_col_letter = openpyxl.utils.get_column_letter(len(head_cols) + len(columns) + 2)
        if dealers:
            untaxed_formula = f'=SUM({untaxed_col_letter}{first_data_row}:{untaxed_col_letter}{last_data_row})'
            taxed_formula = f'=SUM({taxed_col_letter}{first_data_row}:{taxed_col_letter}{last_data_row})'
            # 2026-08-31同步修正：每個內部料號的數量小計（col_totals）原本也是Python端算好的
            # 靜態數字，一併改成=SUM(...)公式，理由同上——使用者要能點開「總計」列任何一格
            # 都看得到公式、逐欄核對。
            col_total_formulas = []
            for i in range(len(columns)):
                col_letter = openpyxl.utils.get_column_letter(len(head_cols) + i + 1)
                col_total_formulas.append(f'=SUM({col_letter}{first_data_row}:{col_letter}{last_data_row})')
        else:
            untaxed_formula, taxed_formula = 0, 0
            col_total_formulas = [''] * len(columns)
        ws.append(['總計'] + [''] * (len(head_cols) - 1) + col_total_formulas + [untaxed_formula, taxed_formula])
    elif view == 'report':
        ws.title = '依財報料號品名彙總'
...
def _valid_report_names_to_id():
    """回傳 {財報料號品名: 錨點id} 字典——財報料號品名是「多對一」文字（同一品名底下可能
    有多筆internal_code共用），internal_sku_list沒有幫每個品名獨立建一個實體/id，這裡取
    「該品名底下(sort_order,id)最小」的那一筆內部料號id當錨點（跟list_sku()/總表分析等
    既有頁面同一套tie-break排序慣例），供「方案清單」(bookstore_plan_items)／「銷退清單」
    (sales_return_items)存檔時，把使用者選的財報料號品名文字解析成id存起來（2026-09-02
    一勞永逸方案，涵蓋範圍比照[[v1.46]]擴大到這2張表，見dealer_reconcile_schema.sql
    v1.47說明）。之後畫面顯示一律用這個錨點id反查「現在」的report_product_name，料號改名
    （連帶report_product_name跟著變）不會再讓舊資料變孤兒；已知邊界情況見schema v1.47
    的註解（錨點那筆料號被重新歸類到別的品名的極少見情況）。"""
    rows = _fetch_all_rows(lambda: sb.table('internal_sku_list').select('id,report_product_name,sort_order'))
    anchor = {}
    for r in rows:
        name = r.get('report_product_name')
        if not name:
            continue
        sk = (r.get('sort_order') or 0, r.get('id') or 0)
        if name not in anchor or sk < anchor[name][0]:
            anchor[name] = (sk, r['id'])
    return {name: v[1] for name, v in anchor.items()}
...
def _report_name_to_active_codes():
    """回傳 {財報料號品名: [(啟用中的內部料號, id), ...]}（略過is_active=false的料號），
    供「書店」出貨資料比對時判斷：同一財報料號品名底下若只剩1筆啟用中的內部料號，代表沒有
    色號/等級分歧，可以自動比對，不需要方案清單另外指定子料號。2026-09-02修正：每個代碼
    一併帶上internal_sku_list.id（一勞永逸方案），供_resolve_bookstore_sub_sku()自動比對
    時直接拿到id存進matched_main_sku_id，不用之後再靠文字反查一次。"""
    rows = _fetch_all_rows(lambda: sb.table('internal_sku_list').select('id,internal_code,report_product_name,is_active'))
    out = {}
    for r in rows:
        ...
        if name and r.get('internal_code'):
            out.setdefault(name, []).append((r['internal_code'], r.get('id')))
    return out
...
def list_bookstore_plans():
    ...
    if plan_ids:
        ...
        # 2026-09-08銜接v3.124：id優先反查「現在」的財報料號品名/子料號（比照
        # _sales_return_row_with_items()既有寫法），查不到id才退回顯示當初存的文字快照。
        # 這是_valid_report_names_to_id()/report_product_name_id/sub_internal_code_id
        # 這套id關聯改造原本規劃要做、但一直沒有補上的「純顯示位置」（見
        # project_sku_id_based_matching_migration.md「尚待執行」第4點），一併在這裡補齊，
        # 讓下面has_orphan_sku的判斷也能吃到正確反查後的文字，不會誤判「其實有id但文字
        # 快照過期」的資料為孤兒。
        name_by_id, code_by_id = _sku_id_name_code_maps()
        for it in items_rows:
            items_map.setdefault(it['plan_id'], []).append({
                'report_product_name': name_by_id.get(it.get('report_product_name_id')) or it['report_product_name'],
                'amount': it['amount'],
                'sub_internal_code': code_by_id.get(it.get('sub_internal_code_id')) or it.get('sub_internal_code'),
            })
    # 2026-09-08新增：列表頁也一併算出每個方案有沒有「孤兒」拆帳明細（財報料號品名/子料號
    # 對不到目前的內部料號清單，通常是料號改名後v3.122的自動同步條件沒滿足才會殘留），跟
    # 編輯彈窗內既有的「⚠️找不到」判斷邏輯共用同一組_plan_valid_report_names()/
    # _sku_report_name_by_code()查表，使用者不用一筆一筆點開編輯才會發現。
    ...
    for p in plans:
        ...
        p['has_orphan_sku'] = any(
            (it['report_product_name'] not in valid_names)
            or (it.get('sub_internal_code') and sub_name_by_code.get(it['sub_internal_code']) != it['report_product_name'])
            for it in p['items']
        )
...
def get_bookstore_plan(pid):
    ...
    # 2026-09-08銜接v3.124：同list_bookstore_plans()，id優先反查「現在」文字，讓編輯彈窗
    # 打開時看到的品名/子料號是目前主檔的最新狀態，不是存檔當下的舊快照。
    name_by_id, code_by_id = _sku_id_name_code_maps()
    plan['items'] = [{
        'report_product_name': name_by_id.get(i.get('report_product_name_id')) or i['report_product_name'],
        'amount': i['amount'],
        'sub_internal_code': code_by_id.get(i.get('sub_internal_code_id')) or i.get('sub_internal_code'),
...
def _notify_new_bookstore_plan(plan: dict):
    """新增書店經銷商方案（新增方案按鈕，非批次匯入）後寄送通知信（2026-08-31新增，
    使用者明確要求：從2026-08-31程式更新起，若有新增方案，寄送通知信到qq8298@gmail.com）。
    主旨格式：[方案編號][方案中文名稱]_[新增人員][新增日期]（新增日期採台灣時間UTC+8）。
    寄信失敗絕不能影響方案本身已經新增成功這件事，所以例外統一由呼叫端(create_bookstore_plan)接住，
    這裡本身不再另外包try/except。"""
    add_date = (datetime.utcnow() + timedelta(hours=8)).strftime('%Y-%m-%d')
    plan_code = plan.get('plan_code') or ''
    plan_name = plan.get('plan_name') or ''
    creator = plan.get('created_by_name') or ''
    subject = f'[{plan_code}][{plan_name}]_{creator}{add_date}'
    body = (
        f'系統新增一筆書店經銷商方案，內容如下：\n\n'
        f'方案編號：{plan_code}\n'
        f'方案中文名稱：{plan_name}\n'
        f'售價(含稅)：{plan.get("price_incl", "")}\n'
        f'新增人員：{creator}\n'
        f'新增日期：{add_date}\n\n'
        f'（此信件由系統自動寄發，請勿直接回覆）'
    )
    print(f'[書店方案新增通知] 準備寄信 → qq8298@gmail.com | 主旨：{subject}')
    ok, err = _send_notification_mail(['qq8298@gmail.com'], subject, body)
    if not ok:
        print(f'[書店方案新增通知] 寄信失敗：{err}')
...
def create_bookstore_plan():
    ...
    # 2026-09-02新增：同步存id關聯欄位（一勞永逸方案，見_valid_report_names_to_id()說明）
    name_to_id = _valid_report_names_to_id()
    code_to_id = _valid_sku_codes()
    item_rows = [{
        ...
        'report_product_name_id': name_to_id.get(_norm(it.get('report_product_name'))),
        'sub_internal_code_id': code_to_id.get(_norm(it.get('sub_internal_code')) or None),
    ...
    try:
        _notify_new_bookstore_plan(plan)
    except Exception as e:
        print(f'[書店方案新增通知] 例外：{e}')
...
def export_bookstore_plans():
    ...
    for i, n in enumerate(names):
        ...
        for j, (code, _cid) in enumerate(codes):
...
def batch_import_bookstore_plans():
    ...
    valid_names = _plan_valid_report_names()
    # 2026-09-02新增：同步準備id關聯查詢表（一勞永逸方案），供下面組item_rows/
    # all_update_items時把canonical化後的品名/子料號文字解析成id存起來。
    name_to_id = _valid_report_names_to_id()
    code_to_id = _valid_sku_codes()
    canonical_by_key = {}
    dup_key_names = set()
        ...
        for c in chunk:
            ...
            for it in c['items']:
                item_rows.append({'plan_id': pid, 'report_product_name': it['report_product_name'],
                                   ...
                                   'report_product_name_id': name_to_id.get(it['report_product_name']),
                                   'sub_internal_code_id': code_to_id.get(it.get('sub_internal_code'))})
...
# 其他列照常新增（不覆蓋既有資料），比照「上傳團體訂單含發票」批次匯入(v1.20)的dup處理慣例——
...
def _compute_bookstore_order_matrix(rows, col_map):
    ...
    sku_rows = _fetch_all_rows(lambda: sb.table('internal_sku_list')
                                .select('id,report_product_name,purchase_price_notax,sort_order,is_active,is_accessory')
                                .order('sort_order'))
    # 2026-09-02新增：id->目前財報料號品名（一勞永逸方案），供下面items_by_name優先用
    # bookstore_plan_items.report_product_name_id反查「現在」的品名，不受該筆方案項目
    # 存的report_product_name文字快照是否隨料號改名變孤兒影響——這是這個函式（固定欄位
    # 樞紐設計）最需要保護的地方：文字反查落空的話，該筆金額會整個對不到任何一欄，等於
    # 從「書店訂單拆帳」/「總表分析」報表悄悄消失，跟[[v1.46]]修正前的
    # _compute_dealer_batch_summary_pivot()是同一種bug。
    name_by_id = {s['id']: s['report_product_name'] for s in sku_rows if s.get('id') and s.get('report_product_name')}
    cost_by_name = {}
    inconsistent_names = set()
    active_by_name = {}
    ...
    for row in rows:
        ...
        items_by_name = {}
        for it in items:
            name = name_by_id.get(it.get('report_product_name_id')) or it['report_product_name']
    ...
    return {
        ...
        'items_map': items_map,
        # 2026-09-02新增：id->目前財報料號品名（一勞永逸方案），供_parse_bookstore_
        # shipment()沿用同一份查詢結果傳給_resolve_bookstore_sub_sku()，不用另外重查。
...
def _resolve_bookstore_sub_sku(plan_id, name, items_map, name_to_active_codes, name_by_id=None):
    ...
    回傳 (matched_main_sku, matched_main_sku_id, match_status, unmatched_reason)。

    2026-09-02修正（一勞永逸方案）：
    - items_map篩選改用「id優先反查出的目前品名」跟name比對（name_by_id參數），不再只比較
      bookstore_plan_items.report_product_name這個文字快照——如果這筆方案項目是很久以前
      設定、財報料號品名後來改名過，這裡原本會比對失敗、白白漏掉這筆子料號指定，改用id
      反查後即使文字快照孤兒了，仍能正確找到對應的方案項目。
    - 連帶把sub_internal_code_id／啟用料號自己的id一併回傳，讓呼叫端可以直接存進
      matched_main_sku_id，不用之後再靠文字反查一次（呼應dealer_sku_mapping那次修正）。"""
    name_by_id = name_by_id or {}
    items = [it for it in (items_map.get(plan_id) or [])
             if (name_by_id.get(it.get('report_product_name_id')) or it.get('report_product_name')) == name]
    sub_item = next((it for it in items if it.get('sub_internal_code')), None)
    if sub_item:
        return sub_item.get('sub_internal_code'), sub_item.get('sub_internal_code_id'), 'auto_matched', None
    codes = name_to_active_codes.get(name) or []
    if len(codes) == 1:
        code, cid = codes[0]
        return code, cid, 'auto_matched', None
    if not codes:
        return None, None, 'unmatched', f'財報料號品名「{name}」在「內部料號清單」查無啟用中的內部料號，請先確認料號設定'
...
def _parse_bookstore_shipment(raw_bytes):
    ...
    name_by_id = result.get('name_by_id') or {}
    ...
    for r in result['rows']:
        ...
        for name, cell in (r.get('cells') or {}).items():
            sub, sub_id, status, reason = _resolve_bookstore_sub_sku(plan_id, name, items_map, name_to_active_codes, name_by_id)
            ...
            lines.append({
                ...
                'raw_json': raw,
                '_prematched': True, 'matched_main_sku': sub, 'matched_main_sku_id': sub_id,
                'match_status': status,
            })
    ...
    for u in (result.get('unmatched') or []):
        ...
        lines.append({
            ...
            '_prematched': True, 'matched_main_sku': None, 'matched_main_sku_id': None, 'match_status': 'unmatched',
        })
    return lines
...
def bookstore_batch_diff_apply(bid):
    ...
    for it in items:
        ...
        main_sku_v = it.get('matched_main_sku')
        line_recs.append({
            ...
            'matched_main_sku': main_sku_v,
            'matched_gift_sku1': None, 'matched_gift_sku2': None, 'matched_gift_sku3': None,
            'matched_main_sku_id': code_to_id.get(main_sku_v) if main_sku_v else None,
            'matched_gift_sku1_id': None, 'matched_gift_sku2_id': None, 'matched_gift_sku3_id': None,
...
def export_bookstore_orders():
    ...
    col_letter_last = ws4.cell(1, total_col2 - 1).column_letter
    r = 4
    # 2026-08-31新增：Stacy要求「總表分析」頁籤下方的「料號加總」「金額合計(未稅)」/
    # 「金額小計(含稅)」列也要含公式讓她驗證（原本這3列都是Python算好的死數字）。這裡先
    # 記錄每個區塊(境內/境外)每一組「單價列/數量列」實際落在Excel哪兩列，供下面組公式用。
    region_tier_rows = {'境內': [], '境外': []}
    for region in ('境內', '境外'):
        ...
        for t in range(max_t):
            ...
            ws4.cell(r_price, total_col2,
                     f'=SUMPRODUCT({col_letter_first}{r_price}:{col_letter_last}{r_price},'
                     f'{col_letter_first}{r_qty}:{col_letter_last}{r_qty})')
            region_tier_rows[region].append((r_price, r_qty))

    # 2026-08-31新增：「料號加總(境內+境外)」列改成公式——代銷商品/電子書方案這2個集合類
    # 品名，數量小計實際上是「單價」列的加總（該欄的price本身就是聚合後的金額，qty固定1，
    # 見_compute_bookstore_order_pivot()說明），一般料號則是「數量」列的加總。用SUMIF依B欄
    # 的列標籤篩選，"單價*"用萬用字元同時吃到「單價(未稅)」「單價(含稅)」兩種標籤
    # （境內/境外標籤不同），一次涵蓋兩個區塊、不需要分開寫。
    tier_row_first, tier_row_last = 4, r - 1
    ws4.cell(r, 2, '料號加總(境內+境外)')
    for ci, name in enumerate(columns, data_start):
        col_l = ws4.cell(1, ci).column_letter
        if name in pivot['aggregate_names']:
            formula = (f'=SUMIF($B${tier_row_first}:$B${tier_row_last},"單價*",'
                       f'{col_l}{tier_row_first}:{col_l}{tier_row_last})')
        else:
            formula = (f'=SUMIF($B${tier_row_first}:$B${tier_row_last},"數量",'
                       f'{col_l}{tier_row_first}:{col_l}{tier_row_last})')
        _set_int_cell(ws4, r, ci, formula)
    _set_int_cell(ws4, r, total_col2, f'=SUM({col_letter_first}{r}:{col_letter_last}{r})')
    for ci in range(1, total_col2 + 1):
        cc = ws4.cell(r, ci)
        cc.font = totals_font
        cc.fill = totals_fill
    ...
    for region in ('境內', '境外'):
        ...
        # 2026-08-31新增：「金額合計(未稅)」/「金額小計(含稅)」列也改成公式——每個品名欄位
        # 在這個區塊的金額，就是上面該區塊每一組「單價列×對應數量列」逐組相乘後加總（跟
        # _compute_bookstore_order_pivot()裡col_region_total的算法完全對應：
        # sum(t['price']*t['qty'] for t in tiers)）。用region_tier_rows記錄的實際列號，
        # 組出「C4*C5+C6*C7+...」這種一目了然、能直接肉眼核對每一組單價x數量的公式；超過
        # 這個品名實際單價分組數的那幾組，儲存格是空白，空白相乘在Excel裡等於0，不影響加總。
        pairs = region_tier_rows[region]
        for ci, name in enumerate(columns, data_start):
            col_l = ws4.cell(1, ci).column_letter
            if pairs:
                formula = '=' + '+'.join(f'{col_l}{p}*{col_l}{q}' for p, q in pairs)
            else:
                formula = 0
            _set_int_cell(ws4, r, ci, formula)
        _set_int_cell(ws4, r, total_col2, f'=SUM({col_letter_first}{r}:{col_letter_last}{r})')
...
# ============================================================
# ⑩ 書店經銷商 — 上傳團體訂單含發票（純登記查詢用，不跟訂單/方案清單比對金額）
# ============================================================
...
# ============================================================
# 2026-08-12新增：「上傳團體訂單含發票」出貨單附件（可多筆，PDF/.docx/.odt，單檔≤2MB）
# ============================================================
...
def list_group_invoices():
    # 2026-09-10調整：Excel批次匯入功能已移除（只留「新增發票」單筆新增+搬移自「上傳團體
    # 發票檔」的資料），原本依「匯入順序」(last_imported_at/import_seq)排序的邏輯已經沒有
    # 意義（現在幾乎所有資料的這2個欄位都是空的）。改成Stacy要求的「預設按新增日期最新排
    # 最上面」：直接依created_at（新增日期）新到舊排序，同一秒建立的用id當次要排序鍵。
    rows = _fetch_all_rows(lambda: sb.table('bookstore_group_invoices').select('*')
                            .order('created_at', desc=True, nullslast=True)
                            .order('id', desc=True))
...
# 2026-08-27新增：「上傳團體訂單含發票」列表勾選匯出——使用者要求在列表頁加勾選欄，勾選要匯出
# 的發票列後按「匯出」，只匯出勾選到的那幾筆（而不是整批全匯出），供人工核對/寄送用。
# 欄位比照列表頁畫面實際顯示的欄位，不含PDF/出貨單附件本身（附件請直接從列表頁個別下載）。
@dealer_bp.route('/api/bookstore/group-invoices/export', methods=['GET'])
@bookstore_invoice_required
def export_group_invoices():
    ids_param = (request.args.get('ids') or '').strip()
    if not ids_param:
        return jsonify({'error': '請先在列表頁勾選要匯出的發票'}), 400
    try:
        ids = [int(x) for x in ids_param.split(',') if x.strip()]
    except ValueError:
        return jsonify({'error': 'ids 參數格式錯誤'}), 400
    if not ids:
        return jsonify({'error': '請先在列表頁勾選要匯出的發票'}), 400

    rows = _fetch_all_rows(lambda: sb.table('bookstore_group_invoices').select('*').in_('id', ids))
    # 2026-08-27修正：'recon_seq_no'（拆帳編號）不是bookstore_group_invoices資料表本身的
    # 欄位，而是像list_group_invoices()一樣要另外呼叫_attach_recon_seq_info()反查
    # dealer_reconcile_batches才會有值——漏呼叫的話，即使下面ws.append()有寫
    # r.get('recon_seq_no')，匯出的Excel「拆帳編號」欄還是會整欄空白（跟列表頁畫面上
    # 看到的值對不起來），使用者反映匯出檔缺這欄資料就是這個原因。
    _attach_recon_seq_info(rows)
    # 依畫面上的原始順序輸出（勾選順序不一定等於id順序，這裡改用使用者勾選時的id順序，
    # 比對照畫面呈現更直覺——ids本身就是前端依畫面目前排序收集到的勾選順序）。
    ...
    ws = wb.active
    ws.title = '上傳團體訂單含發票'
    ws.append(['銷售單位', '經銷商', 'OA訂單編號', '發票日期', '發票號碼', '價格(含稅)', '未稅價',
               '出貨訂單編號', '應收憑單/請購單號', '備註', '對帳區間', '已寫入對帳', '拆帳編號'])
    for r in ordered_rows:
        ws.append([
            r.get('seller_name'), r.get('dealer_name'), r.get('order_no'), r.get('invoice_date'),
            r.get('invoice_no'), r.get('amount_incl'), r.get('amount_notax'),
            r.get('shipping_order_no'), r.get('receivable_doc_no'), r.get('note'), r.get('recon_period'),
            ('已寫入' if r.get('reconciled_at') else '尚未匯入'), r.get('recon_seq_no'),
        ])

    buf = io.BytesIO()
    wb.save(buf)
    buf.seek(0)
    fname = f"上傳團體訂單含發票_匯出_{now_str()[:10].replace('-', '')}.xlsx"
    return send_file(buf, as_attachment=True, download_name=fname,
                      mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
...
def _auto_attach_pending_staging(invoice_id, invoice_no, user_id=None, user_name=None):
    ...
    try:
        ...
        # 不會再各自呼叫一次insert()，避免「上傳團體訂單含發票」列表同一張發票的PDF重複顯示。
...
def create_group_invoice():
    ...
    # 2026-09-10新增：「上傳團體發票檔」併入本模組後，Stacy要求OA訂單編號改為必填
    # （原本選填，因為過去發票PDF靠Gmail自動抓，不一定馬上知道OA訂單編號；現在改成
    # 上傳發票PDF時就要求先填好，才能拿這個編號跟「上傳團體出貨單」比對）。
    order_no = _norm(data.get('order_no'))
    if not order_no:
        return jsonify({'error': 'OA訂單編號為必填'}), 400
...
def update_group_invoice(iid):
    ...
    if amount_incl is None:
        return jsonify({'error': '價格(含稅)為必填，且需為數字'}), 400
    # 2026-09-10新增：同create_group_invoice()，OA訂單編號改為必填（見該函式註解）。
    order_no = _norm(data.get('order_no'))
    if not order_no:
        return jsonify({'error': 'OA訂單編號為必填'}), 400
    invoice_no = _norm(data.get('invoice_no')) or None
...
# ── 上傳團體訂單含發票 — 批次匯入範本下載（比照全系統慣例：必填欄位黃底、說明列、防呆範例列） ──
...
def group_invoice_template():
    ...
    wb.save(buf)
    buf.seek(0)
    return send_file(buf, as_attachment=True, download_name='團體訂單資訊匯入範本.xlsx',
                      mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')


# ── 上傳團體訂單含發票 — 批次匯入（每列皆為新增一筆登記；對帳區間整批共用同一個使用者輸入值） ──
@dealer_bp.route('/api/bookstore/group-invoices/batch-import', methods=['POST'])
@bookstore_invoice_required
def batch_import_group_invoices():
    # 2026-09-10移除：Stacy要求「上傳團體訂單含發票」（改名後：上傳團體訂單含發票）拿掉
    # Excel批次匯入，只留單筆新增（改成先上傳發票PDF、自動讀欄位回填的新流程，跟Excel
    # 批次匯入的操作方式衝突，且OA訂單編號/發票PDF都改成上傳當下就要處理，不適合再用
    # Excel一次匯入一堆列）。前端已拿掉「批次匯入」卡片/按鈕，這裡比照Gmail停用的做法，
    # 程式碼保留、只在API層擋下來，不整段刪除（萬一之後需要retrieve歷史邏輯還找得到）。
    return jsonify({'error': '批次匯入功能已於2026/9/10移除，請改用「新增發票」單筆新增'}), 400
    if 'file' not in request.files:
        return jsonify({'error': '未上傳檔案'}), 400
    f = request.files['file']
...
# ============================================================
# 2026-08-12新增：「上傳團體訂單含發票」Gmail自動抓發票PDF排程（限admin使用）
# ============================================================
...
# ------------------------------------------------------------
# 2026-09-10停用：Stacy要求「發票PDF」改從「上傳團體發票檔」以OA訂單編號比對掛檔，
# 這一整套Gmail自動抓發票PDF機制全面停用（已用AskUserQuestion向Stacy確認：停用但
# 保留程式碼，OA訂單編號自動掛檔的部分則先暫停、尚未開發）。
# 停用做法：
#   ① app.py不再呼叫start_gmail_sync_scheduler()，背景排程不會啟動。
#   ② 這裡加一支旗標，讓start_gmail_sync_scheduler()本身也自我檢查（雙重保險，避免
#      日後不小心恢復app.py那行呼叫卻忘記拿掉這個旗標）。
#   ③ 「立即執行一次」「儲存設定」這兩支會真正觸發動作的API也加上檢查直接擋下來；
#      GET設定/執行紀錄兩支唯讀API維持可查閱，方便之後回頭查歷史設定/紀錄。
#   ④ 前端畫面（templates/dealer_reconcile.html的gmailSyncBox）已改成唯讀停用提示，
#      拿掉啟用開關/抓取時間/儲存設定/立即執行等操作項目，只保留歷史執行紀錄可展開查看；
#      模組標題（page-sub）加註「2026/9/10停用」文字。
# 之後如需恢復：把下面_GMAIL_SYNC_DISABLED改回False、app.py取消註解那行呼叫、
# 前端gmailSyncBox的UI操作項目復原即可，程式邏輯本身完全沒有被刪除。
_GMAIL_SYNC_DISABLED = True
_GMAIL_SYNC_DISABLED_MSG = 'Gmail自動抓發票PDF已於2026/9/10停用，請改用「上傳團體發票檔」以OA訂單編號比對掛檔'
# ============================================================
...
# 2026-08-27新增：使用者反饋轉寄信常被手動改主旨（例如改成「發票1」「發票2」），導致
# 上面主旨關鍵字比對(_GMAIL_SUBJECT_KEYWORD)+_INVOICE_NO_RE(從主旨結尾括號擷取發票號)
# 完全失效、整封信被略過。改為放寬條件：不再檢查主旨，只要信件有PDF附件，且附件檔名
# 或PDF文字內容裡找得到「發票號碼格式」（統一發票電子發票號碼固定格式＝2碼大寫英文字母
# +8碼數字，如DE51907395），就當作候選發票處理，跟主旨文字完全無關（見下方
# _process_one_invoice_email()的新寫法）。_GMAIL_SUBJECT_KEYWORD／_INVOICE_NO_RE這兩個
# 舊常數不再被使用，保留是因為程式其他地方沒有引用、不影響運作，且保留舊寫法可對照。
# 2026-09-10修正：Stacy實測「上傳團體訂單含發票」的發票PDF自動讀取功能，回報明明PDF上
# 看得到發票號碼「FG-51907217」，系統卻讀不到——根因是電子發票證明聯印出來的號碼中間
# 有一個連字號（字軌2碼英文字母 + "-" + 8碼流水號），跟Gmail附件檔名慣用的無連字號寫法
# （如DE51907395.pdf）不一樣，原本的正則沒有考慮這個連字號，比對PDF內文文字時就抓不到。
# 改成連字號可有可無都比對得到，抓到後在_extract_invoice_pdf_fields()裡把連字號拿掉，
# 讓存進資料庫的發票號碼格式跟其他地方（如Gmail檔名）一致，不含連字號。
_INVOICE_NO_ATTACH_RE = re.compile(r'[A-Z]{2}-?\d{8}')
...
def _run_gmail_invoice_sync(trigger_type='scheduled', user_id=None, user_name=None):
    ...
    try:
        ...
        def _process_one_invoice_email(uid_bytes, uid):
            ...
            # 2026-08-27改寫：不再先看主旨關鍵字，直接抓整封信（含附件），理由是使用者
            # 反饋轉寄信常被手動改主旨（如改成「發票1」），主旨比對會把這些轉寄信漏掉。
            # 效能取捨：這裡犧牲了原本「主旨不符合就不下載附件」的省流量設計，改成每封
            # 新信都會抓整封信內容——考量這個信箱平常新信量不大（水位機制只處理「上次
            # 執行之後的新信」），可接受；若之後信箱新信量變大導致單次執行逼近逾時，
            # 可以再考慮加回「寄件人網域」之類的輕量前置過濾條件。
            ...
            # 找出這封信裡所有PDF附件（用副檔名.pdf或Content-Type=application/pdf判斷，
            # 涵蓋轉寄信附件檔名被信箱系統改過大小寫/加流水號的情況）。一封信可能同時
            # 附多張發票PDF（例如轉寄時把好幾張一起夾在同一封信），逐一嘗試辨識發票號碼、
            # 各自獨立處理，不因其中一個附件辨識失敗就整封信放棄。
            pdf_parts = []
            for part in msg.walk():
                fname = part.get_filename()
                ctype = (part.get_content_type() or '').lower()
                if fname:
                    fname_dec = _decode_mime_words(fname).strip()
                else:
                    fname_dec = ''
                if not (fname_dec.lower().endswith('.pdf') or ctype == 'application/pdf'):
                    continue
                pdf_parts.append((fname_dec, part))
            if not pdf_parts:
                return  # 這封信沒有PDF附件，跟這個功能無關，不計入「掃到」

            scanned += 1
            found_any = False

            for fname_dec, part in pdf_parts:
                pdf_bytes = part.get_payload(decode=True)
                if not pdf_bytes:
                    continue

                # 發票號碼格式＝2碼大寫英文字母+8碼數字（如DE51907395，統一發票電子發票
                # 號碼固定格式）。優先比對附件檔名（多數轉寄信檔名本身就是發票號.pdf或
                # 內含發票號）；檔名比對不到才退而求其次，抓PDF內文文字再比對一次
                # （pdfplumber，只讀前3頁，避免大檔案拖慢執行時間——發票號通常在第1頁）。
                invoice_no = None
                m = _INVOICE_NO_ATTACH_RE.search(fname_dec.upper())
                if m:
                ...
                if not invoice_no:
                    try:
                        ...
                        m2 = _INVOICE_NO_ATTACH_RE.search(text.upper())
                        if m2:
                            invoice_no = m2.group(0)
                    except Exception:
                        pass  # PDF損毀/無法解析，當作這個附件找不到發票號，繼續處理下一個附件
                if not invoice_no:
                    continue
                found_any = True
                ...
                # 2026-08-14新增：使用者回報「發票PDF」欄位出現同一個檔名重覆兩份的情況
                # （例如同一張發票的通知信被系統重寄/轉寄了第二次）。修正原則：這張發票
                # 如果已經有一筆同檔名的發票PDF（不論來源是Gmail自動掛檔還是暫存區手動
                # 附加），代表已經比對過、掛過檔了，這裡直接跳過，不再重覆上傳/掛檔。
            ...
            if not found_any:
                error_msg = f'信件「{subject}」的PDF附件檔名/內容都找不到發票號碼格式，略過'
...
def start_gmail_sync_scheduler():
    ...
    insert失敗而提早跳過。"""
    if _GMAIL_SYNC_DISABLED:
        print('[gmail-sync] 已於2026-09-10停用（見_GMAIL_SYNC_DISABLED），略過背景排程啟動')
        return
    try:
...
def update_gmail_sync_config():
    if _GMAIL_SYNC_DISABLED:
        return jsonify({'error': _GMAIL_SYNC_DISABLED_MSG}), 400
...
# ── API：發票PDF暫存區 ──
# 2026-08-27改：list/attach/ignore這3支原本鎖admin_required（跟Gmail帳密/排程設定同一
# 層級），但使用者反饋「從暫存區選取附加」其實是嵌在「新增/編輯發票」表單裡的日常編輯
# 動作，不是系統管理設定，非admin但有團體訂單資訊編輯權限的帳號卻完全用不到、選不到檔案。
# 改成比照「上傳團體訂單含發票」本身的權限（bookstore_invoice_required：mod_dealer_edit或
# mod_bookstore_invoice_view），有權限編輯發票的人就能用，不用額外開通系統管理員身分。
# Gmail帳密/排程設定本身（config/run-now/logs等API）維持admin_required不變。
...
# ── API：發票PDF（已掛在某筆「上傳團體訂單含發票」列上的檔案）──
...
# ============================================================
# 2026-09-10新增：「發票PDF」改為使用者直接上傳＋自動讀取欄位（取代已停用的Gmail自動抓取，
# 見上方_GMAIL_SYNC_DISABLED區塊），同時是「上傳團體發票檔」（bookstore_invoice_uploads）
# 併入本模組後的一次性資料搬移所共用的抽取邏輯。已用AskUserQuestion向Stacy確認3個關鍵
# 設計點：
#   ① 舊資料搬移：網頁上加「執行搬移」按鈕觸發（見下方migrate_invoice_uploads_to_group_
#      invoices()），不是純程式碼調整，需要實際寫資料庫＋搬檔案。
#   ② 搬移時「價格(含稅)」讀不到、或「名稱」當初沒填（這兩個都對應到本表NOT NULL的
#      seller_name/amount_incl）：整筆跳過，不硬塞預設值，執行完回傳清單列出被跳過的
#      流水號＋原因，交給Stacy事後手動處理（她自己決定要修檔案還是直接用新流程重新輸入）。
#   ③ 新的「單筆新增」流程上傳發票PDF後，讀不到「發票日期／發票號碼／價格(含稅)」的話
#      前端會跳警告視窗，要求Stacy確認/手動補上才能按「儲存」（見templates/
#      dealer_reconcile.html的uploadInvoicePdfForForm()/saveInvoiceFormModal()）。
# ============================================================

# 發票日期在Taiwan電子發票PDF上常見的3種寫法：西元「2026-08-27」／「2026/08/27」，
# 以及民國「115年08月27日」（民國年=西元年-1911）。依序嘗試，抓到第一個比對成功的即可，
# 不強求特定關鍵字在前面（不同經銷商/超商列印的電子發票格式差異很大，只能盡量抓）。
_INVOICE_DATE_RE_LIST = [
    re.compile(r'(\d{4})[-/](\d{1,2})[-/](\d{1,2})'),          # 西元
    re.compile(r'(\d{2,3})年(\d{1,2})月(\d{1,2})日'),           # 民國
]
# 價格(含稅)：抓「總計」「總金額」「應付金額」「銷售額合計」等常見標籤後面的數字（可能有
# 千分位逗號），優先比對這幾個關鍵字，抓不到才退而求其次抓PDF裡最大的一個金額數字（發票
# 內文通常會有小計/稅額/總計等好幾個數字，最大的那個多半就是含稅總額）。
_INVOICE_AMOUNT_LABEL_RE = re.compile(r'(?:總\s*計|總金額|應付金額|銷售額合計|總計金額)\D{0,4}([\d,]{2,12})')
_INVOICE_AMOUNT_ANY_RE = re.compile(r'([\d,]{3,12})')


def _parse_invoice_pdf_date(text):
    for rgx in _INVOICE_DATE_RE_LIST:
        m = rgx.search(text)
        if not m:
            continue
        a, b, c = (int(x) for x in m.groups())
        year = a + 1911 if rgx is _INVOICE_DATE_RE_LIST[1] else a
        try:
            return f'{year:04d}-{b:02d}-{c:02d}'
        except Exception:
            continue
    return None


def _extract_invoice_pdf_fields(pdf_bytes):
    """從發票PDF的前3頁文字，盡量抓出發票日期／發票號碼／價格(含稅)三個欄位，抓不到的
    欄位回傳None（不是拋例外，讓呼叫端統一處理「哪些欄位需要手動補」）。純讀取，不寫入
    任何資料庫/Storage。"""
    result = {'invoice_date': None, 'invoice_no': None, 'amount_incl': None}
    try:
        with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
            text = '\n'.join((p.extract_text() or '') for p in pdf.pages[:3])
    except Exception:
        return result  # 損毀/加密/無法解析的PDF，三個欄位都當作讀不到，交給使用者手動輸入
    m = _INVOICE_NO_ATTACH_RE.search(text.upper())
    if m:
        result['invoice_no'] = m.group(0).replace('-', '')
    result['invoice_date'] = _parse_invoice_pdf_date(text)
    m = _INVOICE_AMOUNT_LABEL_RE.search(text)
    if not m:
        m = _INVOICE_AMOUNT_ANY_RE.search(text)
    if m:
        try:
            result['amount_incl'] = float(m.group(1).replace(',', ''))
        except Exception:
            pass
    return result


@dealer_bp.route('/api/bookstore/group-invoices/extract-invoice-pdf', methods=['POST'])
@bookstore_invoice_required
def extract_invoice_pdf():
    """單筆新增/編輯彈窗選好發票PDF後，前端立刻呼叫這支API——純讀取，不落地存檔、不建立
    任何登記，只回傳從PDF文字抓到的欄位，供表單自動回填；讀不到的欄位回傳null，前端負責
    跳警告請Stacy手動確認/補上（見AskUserQuestion確認過的設計）。"""
    f = request.files.get('file')
    if not f or not f.filename:
        return jsonify({'error': '請選擇檔案'}), 400
    if not f.filename.lower().endswith('.pdf'):
        # 非PDF（例如Word/OpenDocument）無法用pdfplumber讀取內容，直接回傳三個欄位皆
        # 讀不到，前端一樣會走「請手動輸入」的提示，不當成錯誤擋下上傳動作本身。
        return jsonify({'invoice_date': None, 'invoice_no': None, 'amount_incl': None,
                         'note': '非PDF檔案，無法自動讀取內容，請手動輸入'})
    data = f.read()
    fields = _extract_invoice_pdf_fields(data)
    return jsonify(fields)


@dealer_bp.route('/api/bookstore/group-invoices/<int:iid>/invoice-pdf', methods=['POST'])
@bookstore_invoice_required
def upload_group_invoice_pdf(iid):
    """2026-09-10新增：發票PDF改為使用者直接上傳（取代已停用的Gmail自動抓取／暫存區手動
    附加），檔案格式/大小限制沿用出貨單同一組常數。
    2026-09-10晚間修正：原本上傳成功後單純新增一筆bookstore_group_invoice_pdfs
    （source='manual_upload'），舊資料不刪——這是原設計的錯誤預期，實際上「發票PDF」是
    這筆發票登記唯一的一個附件欄位（不像出貨單允許多檔並存），Stacy回報因為PDF讀不到
    發票號碼、反覆重新上傳同一張測試，結果同一筆發票累積了3份一模一樣的發票PDF紀錄。
    改成：每次新上傳前，先把這筆發票登記既有的發票PDF（不論來源，含Gmail舊資料/搬移
    過來的）連同Storage實體檔案一起清掉，永遠只保留「最後一次上傳」的這一份。既有已經
    重複的舊資料，另外用一次性清理API處理，見dedupe_group_invoice_pdfs()。"""
    inv = sb.table('bookstore_group_invoices').select('id').eq('id', iid).limit(1).execute()
    if not inv.data:
        return jsonify({'error': '找不到這筆發票登記'}), 404
    f = request.files.get('file')
    if not f or not f.filename:
        return jsonify({'error': '請選擇檔案'}), 400
    if not _shipping_doc_ext_ok(f.filename):
        return jsonify({'error': '僅接受 PDF、Word(.docx)、OpenDocument(.odt) 格式'}), 400
    data = f.read()
    if len(data) > _SHIPPING_DOC_MAX_SIZE:
        return jsonify({'error': '檔案超過2MB上限'}), 400
    if not data:
        return jsonify({'error': '檔案是空的'}), 400
    safe_name = secure_filename(f.filename) or 'file'
    storage_path = f'{iid}/{uuid.uuid4().hex}_{safe_name}'
    try:
        # 先清掉這筆發票既有的舊發票PDF（不論來源），確保永遠只留「最後一次上傳」這一份，
        # 不會像舊版一樣無限疊加重複紀錄。
        old_pdfs = sb.table('bookstore_group_invoice_pdfs').select('id,storage_path') \
            .eq('invoice_id', iid).execute()
        for op in (old_pdfs.data or []):
            try:
                sb.storage(_INVOICE_PDF_BUCKET).remove([op['storage_path']])
            except Exception:
                pass
            sb.table('bookstore_group_invoice_pdfs').delete().eq('id', op['id']).execute()
        sb.storage(_INVOICE_PDF_BUCKET).upload(storage_path, data, f.mimetype or 'application/octet-stream')
        sb.table('bookstore_group_invoice_pdfs').insert({
            'invoice_id': iid, 'file_name': f.filename, 'storage_path': storage_path,
            'file_size': len(data), 'source': 'manual_upload',
        }).execute()
    except Exception as e:
...
@dealer_bp.route('/api/bookstore/group-invoices/dedupe-invoice-pdfs', methods=['POST'])
@admin_required
def dedupe_group_invoice_pdfs():
    """2026-09-10晚間新增（限admin，一次性清理，可重複執行）：修正upload_group_invoice_
    pdf()舊版行為留下的重複資料——同一筆發票登記因為PDF讀不到欄位反覆重新上傳測試，
    bookstore_group_invoice_pdfs意外累積了好幾筆一模一樣的發票PDF附件（新版上傳已改成
    每次自動清掉舊的，不會再發生，見上方函式）。這裡掃描全部資料，同一invoice_id若有
    超過1筆PDF，只保留id最大（=最後一次上傳）的那一筆，其餘連同Storage實體檔案一併
    刪除。可重複執行，沒有重複的話不會動到任何資料。"""
    rows = _fetch_all_rows(lambda: sb.table('bookstore_group_invoice_pdfs')
                            .select('id,invoice_id,storage_path,file_name').order('id'))
    by_invoice = {}
    for r in rows:
        by_invoice.setdefault(r['invoice_id'], []).append(r)
    removed = 0
    details = []
    for invoice_id, prs in by_invoice.items():
        if len(prs) <= 1:
            continue
        prs_sorted = sorted(prs, key=lambda x: x['id'])
        keep = prs_sorted[-1]
        to_remove = prs_sorted[:-1]
        for tr in to_remove:
            try:
                sb.storage(_INVOICE_PDF_BUCKET).remove([tr['storage_path']])
            except Exception:
                pass
            sb.table('bookstore_group_invoice_pdfs').delete().eq('id', tr['id']).execute()
            removed += 1
        details.append({'invoice_id': invoice_id, 'file_name': keep.get('file_name'),
                         'removed_count': len(to_remove)})
    return jsonify({'removed': removed, 'affected_invoices': len(details), 'details': details})


@dealer_bp.route('/api/bookstore/invoice-uploads/migrate-to-group-invoices', methods=['POST'])
@admin_required
def migrate_invoice_uploads_to_group_invoices():
    """一次性資料搬移（限admin觸發，可重複按——已搬過的靠migrated_group_invoice_id判斷
    跳過，不會重複搬）：把「上傳團體發票檔」(bookstore_invoice_uploads)的每一筆，搬成
    「上傳團體訂單含發票」(bookstore_group_invoices)的一筆新登記＋一筆發票PDF附件。
    欄位對照（已用AskUserQuestion向Stacy確認）：
      檔案 → 發票PDF（bookstore_group_invoice_pdfs，實際把檔案從bookstore-shipping-docs
             bucket複製一份到bookstore-invoice-files bucket，兩邊資料庫紀錄各自獨立）
      OA訂單編號 → OA訂單編號(order_no)
      名稱 → 銷售單位(seller_name)
      上傳人員 → 新增人員(created_by/created_by_name)
      上傳日期 → 新增日期(created_at)
    seller_name／amount_incl是bookstore_group_invoices的NOT NULL欄位；上傳當下「名稱」
    留空、或PDF讀不到金額（含非PDF格式的Word/OpenDocument舊檔）的情況，這兩個必填欄位就
    湊不齊，整筆跳過、不搬，累積在skipped清單回傳，交給Stacy事後手動處理，不會用0或假資料
    硬塞。發票日期/發票號碼能不能讀到不影響是否搬移（這兩個在本表不是必填），讀不到就
    留空，之後在列表頁編輯畫面補上即可。"""
    rows = _fetch_all_rows(lambda: sb.table('bookstore_invoice_uploads').select('*').order('id'))
    migrated, skipped, already = [], [], 0
    for r in rows:
        if r.get('migrated_group_invoice_id'):
            already += 1
            continue
        customer_name = _norm(r.get('customer_name'))
        oa_order_no = _norm(r.get('oa_order_no'))
        file_name = r.get('file_name') or ''
        storage_path = r.get('storage_path')
        if not customer_name:
            skipped.append({'id': r['id'], 'file_name': file_name, 'oa_order_no': oa_order_no,
                             'reason': '「名稱」當初上傳時沒填，對應銷售單位為必填欄位，無法搬移'})
            continue
        extracted = {'invoice_date': None, 'invoice_no': None, 'amount_incl': None}
        pdf_bytes = None
        if storage_path:
            try:
                pdf_bytes = sb.storage(_SHIPPING_DOC_BUCKET).download(storage_path)
            except Exception:
                pdf_bytes = None
        if pdf_bytes and file_name.lower().endswith('.pdf'):
            extracted = _extract_invoice_pdf_fields(pdf_bytes)
        if extracted['amount_incl'] is None:
            skipped.append({'id': r['id'], 'file_name': file_name, 'oa_order_no': oa_order_no,
                             'reason': '無法從發票檔自動讀出「價格(含稅)」（非PDF格式或PDF讀不到金額），對應價格(含稅)為必填欄位，無法搬移'})
            continue
        rec = {
            'seller_name': customer_name,
            'order_no': oa_order_no,
            'invoice_date': extracted['invoice_date'],
            'invoice_no': extracted['invoice_no'],
            'amount_incl': extracted['amount_incl'],
            'amount_notax': _round2(extracted['amount_incl'] / 1.05),
            'note': '（由「上傳團體發票檔」搬移，2026-09-10）',
            'created_by': r.get('created_by'),
            'created_by_name': r.get('created_by_name'),
            'created_at': r.get('created_at'),
        }
        try:
            ins = sb.table('bookstore_group_invoices').insert(rec).execute()
            new_id = ins.data[0]['id']
        except Exception as e:
            skipped.append({'id': r['id'], 'file_name': file_name, 'oa_order_no': oa_order_no,
                             'reason': f'建立團體訂單資訊登記失敗：{e}'})
            continue
        if pdf_bytes:
            try:
                new_storage_path = f'{new_id}/{uuid.uuid4().hex}_{secure_filename(file_name) or "file"}'
                sb.storage(_INVOICE_PDF_BUCKET).upload(new_storage_path, pdf_bytes, 'application/octet-stream')
                sb.table('bookstore_group_invoice_pdfs').insert({
                    'invoice_id': new_id, 'file_name': file_name, 'storage_path': new_storage_path,
                    'file_size': r.get('file_size'), 'source': 'migrated_from_upload',
                }).execute()
            except Exception as e:
                skipped.append({'id': r['id'], 'file_name': file_name, 'oa_order_no': oa_order_no,
                                 'reason': f'團體訂單資訊已建立(#{new_id})，但發票PDF檔案搬移失敗：{e}（請手動到該筆補上傳發票PDF）'})
        sb.table('bookstore_invoice_uploads').update({'migrated_group_invoice_id': new_id}).eq('id', r['id']).execute()
        migrated.append({'id': r['id'], 'new_group_invoice_id': new_id, 'file_name': file_name})
    return jsonify({'migrated': migrated, 'skipped': skipped, 'already_done': already})


# ============================================================
# 2026-08-14新增：「上傳團體出貨單」——「上傳團體訂單含發票」的姊妹功能。使用者的出貨單附件
# 目前只能在「上傳團體訂單含發票」的新增/編輯彈窗裡逐筆手動上傳（見上方bookstore_group_invoice_
# files），但出貨單往往比發票早到、或由不同同仁（倉管/物流）先收到，不方便每次都要先找到
# 對應那筆發票才能上傳。這裡新增一個獨立的「上傳團體出貨單」登記頁面，先以「OA訂單編號」
# 為key把出貨單收進來，再由下面的排程「自動比對出貨單檔至團體訂單資訊」，依OA訂單編號比對到
# 「上傳團體訂單含發票」的既有列，自動把出貨訂單編號／出貨單附件寫回對應的發票列——跟「匯入
# 團體訂單資訊」原有的出貨單上傳功能是兩條互補的路徑：這裡是「先收集、後比對」，原本那個是
...
# 2. 比對基數：同一個OA訂單編號如果在「上傳團體訂單含發票」裡對應到多筆發票列，只附加到
...
# 跟「上傳團體訂單含發票」既有的「{invoice_id}/」前綴區隔，避免同樣數字的doc_id/invoice_id
# 互相踩到對方的檔案路徑。
# ============================================================
...
def list_shipping_docs():
    rows = _fetch_all_rows(lambda: sb.table('bookstore_shipping_docs').select('*')
                            .order('id', desc=True))
    # 2026-08-21新增：「封存」勾選+備註——跟「從上傳團體訂單含發票勾選匯入」候選清單同一套設計
    ...
    # 已經寫入對帳，每次都要另外切去「上傳團體訂單含發票」頁面查。這裡依match_status='matched'
    ...
    if matched_ids:
        try:
            ...
            # 比對到的發票日期/發票號碼，不用切去「上傳團體訂單含發票」查。
            inv_res = sb.table('bookstore_group_invoices') \
...
def delete_shipping_doc(did):
    ...
    # delete_group_invoice()既有的做法）。這裡不動已經比對搬過去、掛在「上傳團體訂單含發票」
...
def upload_shipping_doc_files(did):
    ...
    團體訂單資訊」的出貨單附件共用同一個Storage bucket（見上方區塊說明的路徑前綴區隔）。"""
...
# ============================================================
# 2026-08-14新增：「自動比對出貨單檔至團體訂單資訊」排程——比照Gmail自動抓發票PDF排程
...
def _run_shipping_doc_match(trigger_type='scheduled', user_id=None, user_name=None):
    """執行一次「出貨單檔比對到團體訂單資訊」。trigger_type='scheduled'受bookstore_
    ...
    try:
        ...
        for doc in docs:
            ...
            # 避免每次執行都無限疊加重複檔案；使用者手動在「上傳團體訂單含發票」自己上傳的出貨單
...
# ============================================================
# 2026-09-10新增：「上傳團體發票檔」——使用者要求放在「上傳團體出貨單」下方，比照它的
# 查詢/欄位風格，但這裡**純粹是檔案存放與查詢**，跟「上傳團體訂單含發票」/「上傳團體出貨單」
# 完全不連動、沒有任何自動比對機制（已用AskUserQuestion向使用者確認，不要跟上面
# 上傳團體出貨單一樣做OA訂單編號自動比對配對）。
#
# 使用情境：一次選取多個發票檔案（PDF/Word/OpenDocument），系統逐一存檔，
# 每個檔案各自變成清單裡的一列（欄位：流水號/OA訂單編號/名稱/上傳日期），OA訂單編號跟
# 名稱（客戶/單位名稱）欄位上傳當下留空，之後直接在清單列上就地編輯即可（比照「上傳團體
# 出貨單」清單「封存備註」欄的就地編輯做法，不需要另外開彈窗）；單筆上傳＝同一支批次API
# 只選1個檔案，不需要另外寫一條路徑。
#
# 檔案共用既有的bookstore-shipping-docs bucket（跟「上傳團體出貨單」/「上傳團體訂單含發票」
# 出貨單附件同一個bucket，格式/大小限制沿用同一組常數_SHIPPING_DOC_ALLOWED_EXT/
# _SHIPPING_DOC_MAX_SIZE），storage路徑用「invupload-{id}/」前綴跟其他兩種
# （shipdoc-{doc_id}/、{invoice_id}/）區隔，避免互相踩到路徑。
# ============================================================
_BOOKSTORE_INVOICE_UPLOAD_PERM_KEYS = (
    'mod_bookstore_invoice_upload_view', 'mod_bookstore_invoice_upload_create',
    'mod_bookstore_invoice_upload_update', 'mod_bookstore_invoice_upload_delete',
)


def _any_bookstore_invoice_upload_perm():
    return any(_perm_ok(k) for k in _BOOKSTORE_INVOICE_UPLOAD_PERM_KEYS)


def bookstore_invoice_upload_view_required(f):
    """「上傳團體發票檔」檢視權限——比照bookstore_shipping_view_required的萬用總開關
    設計，mod_dealer_edit（既有整體編輯權限）OR 這個功能任一細分權限，即可打開頁面/列表。"""
    @wraps(f)
    def wrapped(*args, **kwargs):
        if not _login_ok():
            return jsonify({'error': '請先登入'}), 401
        if not (_perm_ok('mod_dealer_edit') or _any_bookstore_invoice_upload_perm()):
            return jsonify({'error': '權限不足'}), 403
        return f(*args, **kwargs)
    return wrapped


def bookstore_invoice_upload_action_required(*keys):
    """「上傳團體發票檔」新增/修改/刪除細分權限，各自獨立勾選（比照上傳團體出貨單
    bookstore_shipping_action_required的設計）。"""
    def deco(f):
        @wraps(f)
        def wrapped(*args, **kwargs):
            if not _login_ok():
...
# 2026-09-10新增：這裡的「比對狀態」是純唯讀顯示——依oa_order_no即時反查「上傳團體出貨單」
# （bookstore_shipping_docs.oa_order_no）是否存在同一個值，不像「上傳團體出貨單」自己那套
# match_status有排程自動比對＋寫回matched_invoice_id，這裡完全不寫回任何資料、不影響
# bookstore_shipping_docs，單純讓Stacy一眼看出這個OA訂單編號有沒有對應到已登記的出貨單，
# 方便她自行決定要不要去「上傳團體出貨單」核對。
_INVOICE_UPLOAD_MATCH_STATUS_LABELS = {
    'matched': '已找到對應出貨單', 'no_match': '查無對應出貨單', 'no_oa': '尚未填寫OA訂單編號',
}


@dealer_bp.route('/api/bookstore/invoice-uploads', methods=['GET'])
@bookstore_invoice_upload_view_required
def list_invoice_uploads():
    kw = _norm(request.args.get('kw', ''))
    rows = _fetch_all_rows(lambda: sb.table('bookstore_invoice_uploads').select('*')
                            .order('id', desc=True))
    if kw:
        kw_low = kw.lower()
        rows = [r for r in rows if kw_low in (r.get('oa_order_no') or '').lower()
                or kw_low in (r.get('customer_name') or '').lower()
                or kw_low in (r.get('file_name') or '').lower()]
    oa_nos = {_norm(r.get('oa_order_no')) for r in rows if _norm(r.get('oa_order_no'))}
    matched_oa_nos = set()
    if oa_nos:
        try:
            sdoc_res = sb.table('bookstore_shipping_docs').select('oa_order_no') \
                .in_('oa_order_no', list(oa_nos)).execute()
            matched_oa_nos = {_norm(sd.get('oa_order_no')) for sd in (sdoc_res.data or [])}
        except Exception:
            matched_oa_nos = set()
    for r in rows:
        oa = _norm(r.get('oa_order_no'))
        status = 'no_oa' if not oa else ('matched' if oa in matched_oa_nos else 'no_match')
        r['match_status'] = status
        r['match_status_label'] = _INVOICE_UPLOAD_MATCH_STATUS_LABELS.get(status)
    return jsonify(rows)


@dealer_bp.route('/api/bookstore/invoice-uploads/batch', methods=['POST'])
@bookstore_invoice_upload_action_required('mod_bookstore_invoice_upload_create')
def batch_upload_invoice_files():
    """一次可選多檔，每個檔案各自新增一筆登記＋各自獨立檢查格式/大小，部分失敗不擋下
    其他檔案（比照upload_shipping_doc_files()既有做法）；OA訂單編號在上傳當下由前端表單
    帶入(同一批選取的檔案套用同一個編號，2026-09-10改為上傳前填寫，方便之後自行拿這個
    欄位跟「上傳團體訂單含發票」核對)，名稱則維持留空，之後在列表頁就地編輯。單筆上傳沿用同一支
    API，只選1個檔案即可。"""
    files = request.files.getlist('files')
    if not files:
        return jsonify({'error': '請選擇要上傳的檔案'}), 400
    oa_order_no = _norm(request.form.get('oa_order_no')) or None
    uploaded, errors = [], []
    for f in files:
        if not f or not f.filename:
            continue
        ...
        try:
            rec = {
                'file_name': f.filename, 'storage_path': '',  # 先佔位，拿到id後補上正確路徑
                'file_size': len(data),
                'oa_order_no': oa_order_no,
                # 2026-09-10修正：這張表只有created_by/created_by_name兩個稽核欄位（沒有
                # updated_by/updated_by_name，因為這個功能不像其他模組有「修改人員」概念），
                # 原本用_audit_new()一次塞4個欄位會因為多出schema沒有的updated_by/
                # updated_by_name兩欄，被Supabase以PGRST204拒絕整筆insert（Stacy實測上傳
                # 直接看到「Could not find the 'updated_by' column」錯誤）。改成只手動帶入
                # 這張表實際擁有的2個欄位。
                'created_by': session.get('user_id'),
                'created_by_name': session.get('display_name', session.get('username', '')),
            }
            ins = sb.table('bookstore_invoice_uploads').insert(rec).execute()
            new_id = ins.data[0]['id']
        except Exception as e:
            errors.append(f'{f.filename}：建立登記失敗（{e}）')
            continue
        safe_name = secure_filename(f.filename) or 'file'
        storage_path = f'invupload-{new_id}/{uuid.uuid4().hex}_{safe_name}'
        try:
            sb.storage(_SHIPPING_DOC_BUCKET).upload(
                storage_path, data, f.mimetype or 'application/octet-stream')
            sb.table('bookstore_invoice_uploads').update(
                {'storage_path': storage_path}).eq('id', new_id).execute()
            row = sb.table('bookstore_invoice_uploads').select('*').eq('id', new_id).limit(1).execute()
            uploaded.append(row.data[0])
        except Exception as e:
            # 上傳失敗，把剛剛佔位新增的登記一併刪掉，避免留下一筆沒有檔案的空紀錄
            try:
                sb.table('bookstore_invoice_uploads').delete().eq('id', new_id).execute()
            except Exception:
                pass
            errors.append(f'{f.filename}：上傳失敗（{e}）')
    return jsonify({'uploaded': uploaded, 'errors': errors})
...
@dealer_bp.route('/api/bookstore/invoice-uploads/<int:iid>', methods=['PUT'])
@bookstore_invoice_upload_action_required('mod_bookstore_invoice_upload_update')
def update_invoice_upload(iid):
    """就地編輯OA訂單編號／名稱（客戶/單位名稱），比照「上傳團體出貨單」清單封存備註欄
    的就地編輯做法，不需要另外開彈窗；兩個欄位皆選填，允許清空。"""
    data = request.json or {}
    cur = sb.table('bookstore_invoice_uploads').select('id').eq('id', iid).limit(1).execute()
    if not cur.data:
        return jsonify({'error': '找不到這筆發票檔登記'}), 404
    rec = {}
    if 'oa_order_no' in data:
        rec['oa_order_no'] = _norm(data.get('oa_order_no')) or None
    if 'customer_name' in data:
        rec['customer_name'] = _norm(data.get('customer_name')) or None
    if not rec:
        return jsonify({'ok': True})
    sb.table('bookstore_invoice_uploads').update(rec).eq('id', iid).execute()
    return jsonify({'ok': True})


@dealer_bp.route('/api/bookstore/invoice-uploads/<int:iid>', methods=['DELETE'])
@admin_required
def delete_invoice_upload(iid):
    """限admin使用（比照delete_shipping_doc()），刪除前先清掉Storage裡的實體檔案。"""
    row = sb.table('bookstore_invoice_uploads').select('storage_path').eq('id', iid).limit(1).execute()
    if not row.data:
        return jsonify({'error': '找不到這筆發票檔登記'}), 404
    ...
    if storage_path:
        try:
            ...
            pass
    sb.table('bookstore_invoice_uploads').delete().eq('id', iid).execute()
    return jsonify({'ok': True})


@dealer_bp.route('/api/bookstore/invoice-uploads/<int:iid>/download', methods=['GET'])
@bookstore_invoice_upload_view_required
def download_invoice_upload(iid):
    row = sb.table('bookstore_invoice_uploads').select('*').eq('id', iid).limit(1).execute()
    if not row.data:
        return jsonify({'error': '找不到這筆發票檔登記'}), 404
    rec = row.data[0]
    try:
...
#
# v1.1  2026-09-11：總表欄名比對邏輯改用「內部料號清單」的專屬比對欄位
# ledger_history_sku_name（發票總表比對名稱），比照shipping_system_product_name／
# shipment_qty_diff()同一套設計——使用者反映內部料號本身如果之後改了中文品名，
# 舊總表欄名文字就對不到，每次都要重新匯入。設定這個專屬欄位後，即使internal_code
# 之後改名，這個欄位維持不變，舊總表欄名一樣比對得到；沒設定的料號則自動退回用
# internal_code本身比對，相容所有既有總表檔案，不需要事後回填。詳見_sil_extract_records()
# 的label_map。
# ============================================================
...
def _sil_orphan_label_key(label):
    """「孤兒欄位」（寬表檢視/Excel匯出裡查無對應內部料號的料號欄，見_wide_view_build()）
    的分組鍵——2026-09-11新增。根因：Stacy截圖回報寬表檢視畫面上出現2欄看起來重複、
    表頭跟內容都是空白的欄位，追查後懷疑是同一個料號的Excel表頭文字，因為手動編輯/複製
    貼上殘留的全形空白、頭尾空白或Tab等空白字元差異，被raw_sku_label字串完全比對誤判成
    2個不同的欄位鍵值（過去孤兒欄位的分組鍵一律直接用raw_sku_label原始文字，見下方
    _wide_view_build()舊寫法）。改用_sil_norm_ws()去除所有空白字元後的版本當分組鍵，
    讓這類純空白差異的重複欄位合併成一欄；如果正規化後整段文字變成空字串（表頭本身是
    空白，或含零寬字元等其他Unicode「格式字元」肉眼看不到但不是標準空白），視為無意義的
    欄位，呼叫端會直接跳過不顯示。顯示用的表頭文字仍取第一次出現時的原始raw_sku_label，
    不受正規化影響。"""
    t = _sil_norm_ws(label)
    if not t:
        return ''
    # 額外濾掉零寬空白/BOM等Unicode「格式字元」（Cf類別）——這類字元肉眼完全看不到，
    # 但\s正規表示式不會比對到，str.strip()也不會移除，若不濾掉會讓看起來空白的表頭
    # 被誤判成「非空白」而產生看不到名字的欄位。
    cleaned = ''.join(ch for ch in t if unicodedata.category(ch) != 'Cf')
    return cleaned
...
def _sil_extract_records(ws, sku_cols, extra_cols, verify_row_idx, label_map, source_ref):
    """wide -> long 轉換。label_map：{總表欄名文字 -> internal_code} 對照表（見
    import_invoice_ledger_history()組法：優先用「發票總表比對名稱」(ledger_history_sku_name，
    2026-09-11新增，比照shipping_system_product_name同一套設計)，沒設定的料號才退回用
    internal_code本身當key，兩者同時並存不衝突）。比對到的填入internal_code且
    is_legacy_column=False，比對不到的internal_code=None、is_legacy_column=True（代表這欄
    可能是已歸檔舊料號，或料號主檔還沒補上這個新料號）。"""
    records = []
    ...
    for row_order, row in enumerate(ws.iter_rows(min_row=2, max_row=verify_row_idx - 1, max_col=max_col, values_only=True), start=2):
        ...
        for col_idx, raw_label in sku_cols:
            ...
            if qty is None or not isinstance(qty, (int, float)) or qty == 0:
                continue
            row_has_qty = True
            internal_code = label_map.get(raw_label)
            matched = internal_code is not None
...
def _sil_summarize(records, skipped, sku_cols, valid_codes=None):
    ...
    # 2026-08-31新增：「比對不到」的欄位名稱，額外做模糊比對——commit時這些欄位會被
    # 自動在「內部料號清單」建立佔位資料（見下方import_invoice_ledger_history()），
    # 如果這個欄位名稱其實只是既有料號打字/格式上的落差（少一個字、少個「開完發票」
    # 尾綴等），過去會被當成全新料號默默建檔，之後就變成「對帳查詢→依內部料號名矩陣」
    # 橫軸標題列重複的根因之一。這裡在預覽階段先做相似度比對，找到疑似重複就標記出來，
    # 純軟性警示、不擋匯入——使用者可以自行判斷後回頭修改來源Excel表頭文字，或確認
    # 這真的是全新料號就直接匯入即可。
    SIL_SIMILAR_THRESHOLD = 0.72
    codes_list = sorted(valid_codes) if valid_codes else []
    similar_suspect_count = 0
    if codes_list:
        for entry in all_label_breakdown:
            if not entry['is_legacy_column']:
                continue
            best_ratio, best_code = 0.0, None
            for code in codes_list:
                r = difflib.SequenceMatcher(None, entry['raw_sku_label'], code).ratio()
                if r > best_ratio:
                    best_ratio, best_code = r, code
            if best_ratio >= SIL_SIMILAR_THRESHOLD:
                entry['similar_existing_code'] = best_code
                entry['similar_ratio'] = round(best_ratio, 3)
                similar_suspect_count += 1
    ...
    return {
        ...
        'similar_suspect_count': similar_suspect_count,
    }
...
def import_invoice_ledger_history():
    ...
    2026-09-11更新：欄名比對優先用「內部料號清單」的「發票總表比對名稱」
    (ledger_history_sku_name)，沒設定才退回用internal_code本身比對（見_sil_extract_records()
    的label_map），內部料號改名不再影響舊總表欄名的比對結果。
    ...
    sku_list_rows = _fetch_all_rows(lambda: sb.table('internal_sku_list').select('id,internal_code,ledger_history_sku_name,sort_order'))
    ...
    # 2026-09-11新增：比照shipping_system_product_name/shipment_qty_diff()同一套設計——
    # 總表欄名比對改用專屬欄位「發票總表比對名稱」(ledger_history_sku_name)，內部料號本身
    # 之後改名不影響已經匯入的舊總表欄名比對。label_map同時保留internal_code本身當key
    # （沒設定ledger_history_sku_name的料號、或舊總表欄名本來就等於internal_code的既有
    # 情況都要能繼續比對到），兩者同時存在，後者(ledger_history_sku_name)如果剛好跟別筆
    # 料號的internal_code文字撞名，以先出現者為準——這種撞名應極罕見，不特別處理。
    label_map = {}
    # 2026-09-11新增：ledger_name_by_code記錄每個internal_code目前的ledger_history_sku_name
    # 設定值，供下面commit階段「自動回填」使用——Stacy要求總表欄位名稱要跟系統保持一致，
    # 不要每次匯入後才發現對不到，見下方col_order_payload迴圈的自動回填邏輯。
    ledger_name_by_code = {}
    for r in sku_list_rows:
        code = r.get('internal_code')
        if not code:
            continue
        label_map.setdefault(code, code)
        dedicated = r.get('ledger_history_sku_name')
        ledger_name_by_code[code] = dedicated
        if dedicated:
            label_map.setdefault(dedicated, code)

    source_ref = f'{secure_filename(f.filename) or "歷年每月開發票與未開發票總表.xlsx"}'
    records, skipped = _sil_extract_records(ws, sku_cols, extra_cols, verify_row_idx, label_map, source_ref)
    summary = _sil_summarize(records, skipped, sku_cols, valid_codes)
    ...
    if legacy_labels:
        # 2026-09-11修正：判斷「這個總表欄名還需不需要補建佔位資料」改用label_map
        # （涵蓋internal_code＋發票總表比對名稱兩種比對鍵），不再只查valid_codes
        # （純internal_code集合）——legacy_labels本來就是label_map.get()查不到的欄名，
        # 這裡維持一致的判斷依據，避免日後兩邊各自維護漏同步。
        still_missing = [lbl for lbl in legacy_labels if lbl not in label_map]
        if still_missing:
            ...
            try:
                ...
                # 2026-09-11新增：佔位資料internal_code=lbl本身，label_map也要同步補上，
                # 否則下面col_order_payload用label_map反查這批新佔位欄名會查不到。
                for lbl in still_missing:
                    label_map.setdefault(lbl, lbl)
    ...
    col_order_updated = 0
    ledger_name_backfilled = 0
    try:
        col_order_payload = []
        for col_order, (_, raw_label) in enumerate(sku_cols):
            # 2026-09-11修正：raw_label（總表欄名文字）不再保證等於internal_code——欄名
            # 也可能是比對到「發票總表比對名稱」(ledger_history_sku_name) 這種情況，這裡先用
            # label_map反查回真正的internal_code，再用internal_code查id/回寫，不能再直接
            # 拿raw_label當internal_code使用（否則比對到的id.get(raw_label)一定是None，
            # 這欄的排序就會被誤判成「找不到id」而跳過）。
            resolved_code = label_map.get(raw_label) or raw_label
            row_id = id_by_code.get(resolved_code)
            if row_id is not None:
                ...
                # 2026-09-11新增：Stacy要求「上傳總表」的欄位名稱要跟系統保持一致，不要
                # 每次匯入後才發現對不到——這筆料號目前還沒設定「發票總表比對名稱」
                # (ledger_history_sku_name) 的話，自動把「這次總表這一欄的原始標題文字」
                # 回填進去，等於把「今天這份總表的欄名」永久鎖定成這筆料號的比對錨點，之後
                # 即使Stacy改了internal_code的中文名稱，這份（以及同樣欄名的舊）總表依然
                # 對得到，完全不需要Stacy自己一筆一筆到「料號與規則管理」手動填寫。已經有
                # 設定值的料號不覆蓋，保留Stacy原本手動填寫的內容。
                if not ledger_name_by_code.get(resolved_code):
                    entry['ledger_history_sku_name'] = raw_label
                    ledger_name_by_code[resolved_code] = raw_label
                    ledger_name_backfilled += 1
                col_order_payload.append(entry)
        CHUNK = 300
    ...
    # 2026-09-02修正：一勞永逸方案——存檔時一併把internal_code文字解析成id存起來
    # （internal_code_id），比照dealer_sku_mapping/bookstore_plan_items的寫法，見
    # dealer_reconcile_schema.sql v1.47說明。id_by_code此時已包含上面新建立的佔位料號id。
    for r in records:
        r['internal_code_id'] = id_by_code.get(r['internal_code']) if r.get('internal_code') else None
    ...
    # 2026-09-11新增：回報這次匯入自動幫幾筆料號補上「發票總表比對名稱」，讓Stacy知道
    # 系統已經把這份總表目前的欄名鎖定成永久比對錨點，不用自己再手動填寫。
    summary['ledger_name_backfilled_count'] = ledger_name_backfilled
    if col_order_error:
...
# ============================================================
# 「已開/未開發票總表」— 出貨系統出庫數比對（2026-08-25新增，source_type='shipment_diff'）
# ============================================================
# 背景：使用者上傳出貨系統匯出的「已出貨」報表（wide long-format，一次可涵蓋多個商品區塊，
# 每個區塊標題列為「商品編號:xxx,商品名稱:yyy」，底下接著該商品逐筆出貨明細），依「內部
# 料號清單」新增的「出貨商品Id」(shipping_system_product_id，優先比對，數字代碼較穩定)／
# 「出貨系統商品名稱」(shipping_system_product_name，比對不到Id時才退回用名稱比對) 找出
# 對應的內部料號，把每個(內部料號,客戶類型)的出庫數量加總，跟sku_invoice_ledger目前已
# 累計的數量比較，diff = 這次出庫數彙總 - 目前已累計數；diff≠0才新增一列（可為負），
# 不修改/刪除任何既有列——這是本模組[[比對已開/未開發票新模組]]筆記裡「關鍵背景知識」
# 記載的既有設計原則（「差異寫回規則：diff=明細樞紐加總(缺省0)－現有累計(缺省0)；diff≠0
# 就新增一列」），沿用同一套邏輯只是資料來源換成出貨系統出庫數。
#
# 因為「目前已累計數」本身已經包含前一次寫入的shipment_diff結果，重複上傳同一份檔案不會
# 重複疊加（第二次算出的diff會是0）；只有出貨系統之後真的又多出貨、累計數字變大，才會
# 產生新的正差異，天然具備「只補增量」的效果，不需要額外記錄「上次比對到哪裡」的水位。
#
# 比對不到內部料號的商品區塊（多半是「出貨商品Id」/「出貨系統商品名稱」尚未在內部料號
# 清單填寫對照）不會被計入任何差異，只會在預覽結果的unmatched清單顯示供使用者補建對照，
# 避免比對不到就自行用商品名稱字串當internal_code硬塞——這是這個系統多次修過的同一類
# bug（比對不到就自行新增/硬塞），這裡刻意不重蹈覆轍。

def _ssd_parse_rows(ws):
    """把出貨系統「已出貨」報表解析成明細列清單（原始openpyxl列tuple，未做欄位篩選）。
    2026-08-25使用者用小範圍測試檔`2料驗證.xlsx`實測後發現：出貨系統「已出貨」報表
    根本沒有「商品編號:xxx,商品名稱:yyy」這種分段標題列（原始樣本檔`ProductInvStat...`
    剛好每個商品都有這種分段文字，但那只是報表顯示用、並非固定格式），也完全沒有商品
    編號欄——報表其實就是一張平鋪表格，每一列本身就帶B欄=客戶類型、K欄=商品名稱
    (index10)、L欄=購買數量(index11)，不需要靠分段標題列才知道這列是哪個商品。所以
    解析邏輯只要找到A欄文字為「訂單號」的表頭列就開始收集後面每一列當明細列直接用；
    若報表是像原始樣本檔那種多商品分段格式、表頭列重複出現多次也沒關係，一樣持續收集。
    分段用的「商品編號:...,商品名稱:...」或報表最上方「已出貨」標題文字列，因為其他
    欄位都是空白，會在_ssd_aggregate_rows()的欄位有效性檢查時自然被過濾掉，不需要
    額外辨識或跳過。"""
    rows_out = []
    in_data = False
    for row in ws.iter_rows(values_only=True):
        c0 = row[0] if row else None
        if isinstance(c0, str) and c0.strip() == '訂單號':
            in_data = True
            continue
        if not in_data:
            continue
...
def _ssd_aggregate_rows(rows, name_map):
    """使用者2026-08-25明確指示：「請用excel[商品名稱]與內部料號{出貨系統商品名稱}
    去比對」——出貨系統「已出貨」報表沒有商品編號欄，比對鍵改成單純用每列自己的
    「商品名稱」(K欄)查 internal_sku_list.shipping_system_product_name，查到才依
    (internal_code, customer_type)加總購買數量(L欄)；查不到的依商品名稱彙總進
    unmatched（不會用商品名稱字串硬塞當internal_code，避免系統裡多次修過的「比對
    不到就自行新增」bug類型）。
    回傳 (matched_qty: {(internal_code, customer_type): qty_sum},
        unmatched: [{'product_name':, 'qty_sum':, 'row_count':}, ...]（依qty_sum由
        大到小排序）,
        matched_row_count: 成功比對到的明細列數)。"""
    matched_qty = {}
    unmatched_map = {}
    matched_row_count = 0
    for row in rows:
        pname = row[10] if len(row) > 10 else None
        ct = row[1] if len(row) > 1 else None
        q = row[11] if len(row) > 11 else None
        if not pname or not ct or not isinstance(q, (int, float)) or q == 0:
            continue
        pname = str(pname).strip()
        internal_code = name_map.get(pname)
        if not internal_code:
            u = unmatched_map.setdefault(pname, {'product_name': pname, 'qty_sum': 0, 'row_count': 0})
            u['qty_sum'] += q
            u['row_count'] += 1
            continue
        matched_row_count += 1
        key = (internal_code, str(ct).strip())
        matched_qty[key] = matched_qty.get(key, 0) + q
    unmatched = sorted(unmatched_map.values(), key=lambda u: -u['qty_sum'])
    return matched_qty, unmatched, matched_row_count


@dealer_bp.route('/api/dealer/invoice-ledger/shipment-diff', methods=['POST'])
@dealer_edit_required
def shipment_qty_diff():
    """上傳出貨系統「已出貨」出庫數報表，比對內部料號的「出貨商品Id」/「出貨系統商品
    名稱」，依(內部料號,客戶類型)彙總出庫數量，跟sku_invoice_ledger既有累計數比較，
    diff≠0才新增一列（source_type='shipment_diff'，可為負，不修改/刪除既有列）。

    表單參數：
      file    必填，出貨系統「已出貨」報表xlsx（可一次涵蓋多個商品區塊）
      mode    'preview'（預設）：只解析、比對、彙總，不寫入資料庫
              'commit'：正式把每組差異新增一列到sku_invoice_ledger
    """
    if 'file' not in request.files:
        return jsonify({'error': '未上傳檔案'}), 400
    f = request.files['file']
    ...
    for ws_i in wb.worksheets:
        ssd_rows = _ssd_parse_rows(ws_i)
        if ssd_rows:
            break
    if not ssd_rows:
        return jsonify({'error': '找不到「訂單號」表頭列，請確認上傳的是出貨系統「已出貨」報表格式'}), 400

    sku_rows = _fetch_all_rows(lambda: sb.table('internal_sku_list').select(
        'id,internal_code,report_product_name,shipping_system_product_name'))
    name_map = {r['shipping_system_product_name']: r['internal_code'] for r in sku_rows
                if r.get('shipping_system_product_name') and r.get('internal_code')}
    name_by_code = {r['internal_code']: (r.get('report_product_name') or r['internal_code'])
                    for r in sku_rows if r.get('internal_code')}
    # 2026-09-02修正：一勞永逸方案——id_by_code供下面把這次差異寫回internal_code_id；
    # id_to_code供比對「目前已累計數」時，優先用舊列存好的internal_code_id反查「現在」的
    # internal_code文字，避免料號改名後，既有列的文字快照跟這次重新查到的新文字對不起來、
    # 被誤判成「還沒累計過」而重複多算一次差異（跟_compute_bookstore_order_matrix()等既有
    # 讀取端修正同一套id優先邏輯）。
    id_by_code = {r['internal_code']: r['id'] for r in sku_rows if r.get('internal_code')}
    id_to_code = {r['id']: r['internal_code'] for r in sku_rows if r.get('id') and r.get('internal_code')}

    matched_qty, unmatched, matched_row_count = _ssd_aggregate_rows(ssd_rows, name_map)

    # 目前已累計數：sku_invoice_ledger 依 (internal_code, customer_type) 加總既有全部
    # 來源的數量（excel_import＋先前寫入的shipment_diff），配合上方函式說明的「累加式
    # diff」設計，只抓需要的3個欄位減少資料量。
    existing_rows = _fetch_all_rows(lambda: sb.table('sku_invoice_ledger').select('internal_code,internal_code_id,customer_type,qty'))
    existing_qty = {}
    for r in existing_rows:
        code = id_to_code.get(r.get('internal_code_id')) or r.get('internal_code')
        ct = r.get('customer_type')
        if not code or not ct:
            continue
        key = (code, ct)
        existing_qty[key] = existing_qty.get(key, 0) + (r.get('qty') or 0)

    diffs = []
    for (code, ct), shipped in matched_qty.items():
        existing = existing_qty.get((code, ct), 0)
        diff = shipped - existing
        if diff != 0:
            diffs.append({
                'internal_code': code,
                'report_product_name': name_by_code.get(code, code),
                'customer_type': ct,
    ...
    source_ref = secure_filename(f.filename) or '出貨系統出庫數比對.xlsx'
    DIFF_DISPLAY_CAP = 500
    unmatched_row_count = sum(u['row_count'] for u in unmatched)
    summary = {
        'row_count': matched_row_count + unmatched_row_count,
        'matched_row_count': matched_row_count,
        'unmatched_row_count': unmatched_row_count,
        'unmatched_product_count': len(unmatched),
        'unmatched': unmatched,
        'pair_count': len(matched_qty),
        'diff_count': len(diffs),
        'zero_diff_count': len(matched_qty) - len(diffs),
        'diffs': diffs[:DIFF_DISPLAY_CAP],
        'diff_truncated': len(diffs) > DIFF_DISPLAY_CAP,
        'total_diff_qty': sum(d['diff'] for d in diffs),
    }
    ...
    for d in diffs:
        records.append({
            ...
            'internal_code_id': id_by_code.get(d['internal_code']),
            'raw_sku_label': d['internal_code'],
            'is_legacy_column': False,
            'customer_type': d['customer_type'],
            # 2026-08-25第5輪修正：使用者澄清是要讓這筆差異併入「已開/未開發票總表」
            # 裡該客戶類型既有的「未開發票」彙總列，而不是另外標記一筆獨立資料。
            # _wide_view_build()的merge_by_name合併鍵是(customer_type, reconcile_name,
            # period, period_range)——使用者截圖顯示既有「未開發票」彙總列的規律是
            # reconcile_name＝customer_type本身、period／period_range皆固定為'未開發票'，
            # 這裡完全比照這個規律填寫，讓使用者在寬表檢視勾選「合併成一列」時，這筆
            # shipment_diff差異數會自動跟該客戶類型既有的未開發票數量加總顯示在同一格。
            'reconcile_name': d['customer_type'],
            'period': '未開發票',
            'period_range': '未開發票',
            'note': f'出貨系統出庫數比對差異（來源檔：{source_ref}；出庫數{d["shipped_qty"]} - 既有累計{d["existing_qty"]}）',
            'qty': d['diff'],
            'source_type': 'shipment_diff',
            'source_ref': source_ref,
            'created_by': session.get('user_id'),
            'created_by_name': uname,
        })
...
def _wide_view_build(args):
    ...
    sku_rows = _fetch_all_rows(lambda: sb.table('internal_sku_list').select(
        'id,internal_code,report_product_name,sort_order,hide_in_invoice_ledger,is_active,'
    ...
    # 2026-09-02修正：一勞永逸方案——id_to_code供下面把每筆明細的internal_code_id優先反查
    # 「現在」的internal_code文字，再拿去對應欄位/分組鍵，料號改名後不會再變成查無對應
    # 欄位的孤兒資料（比照_compute_bookstore_order_matrix()既有的name_by_id讀取端修正）。
    id_to_code = {r['id']: r['internal_code'] for r in sku_rows if r.get('id') and r.get('internal_code')}

    ledger_rows = _fetch_all_rows(lambda: sb.table('sku_invoice_ledger').select(
        'internal_code,internal_code_id,raw_sku_label,is_legacy_column,customer_type,reconcile_name,period,'
    ...
    orphan_labels = []
    orphan_label_text = {}  # 正規化key -> 第一次出現時的原始raw_sku_label顯示文字
    seen_orphan = set()
    for r in filtered:
        code = id_to_code.get(r.get('internal_code_id')) or r.get('internal_code')
        ...
        if code and code in col_key_set:
            continue  # 已經是正常欄位
        # 2026-09-11新增：孤兒欄位（查無對應內部料號）改用_sil_orphan_label_key()正規化後的
        # 文字當分組鍵，而不是raw_sku_label原始文字——根因是Excel表頭純空白差異（全形空白/
        # 頭尾空白/Tab等）會被字串完全比對誤判成2個不同欄位，畫面上出現2欄看起來重複、其實
        # 是同一個料號的孤兒欄位（Stacy截圖回報「這兩欄重複可以刪掉」）；正規化後變成空字串
        # 的欄位（表頭本身是空白，或含零寬字元等看不見的內容）直接跳過，不再顯示成一欄
        # 沒有名字的空白欄位。
        key = _sil_orphan_label_key(label)
        if not key:
            continue
        if key in seen_orphan:
            continue
        seen_orphan.add(key)
        orphan_label_text[key] = label
        orphan_labels.append(key)
    for key in orphan_labels:
        columns.append({'key': key, 'label': orphan_label_text[key], 'sub_label': '已歸檔／查無對應料號', 'is_legacy': True})
        col_key_set.add(key)
    ...
    for r in filtered:
        ...
        # 2026-09-11調整：孤兒欄位(code無效/查無對應料號)的分組鍵改用_sil_orphan_label_key()
        # 正規化後的文字，跟上面orphan_labels/columns[]的key算法保持一致，否則兩邊各自算出
        # 不同的key、對不起來，會讓料號欄的值對不到正確的欄位。理由見_sil_orphan_label_key()
        # 上方註解（Excel表頭純空白差異被誤判成2個不同欄位的bug）。
        if code and code in col_key_set and code in sku_by_code:
            col_key = code
        else:
            col_key = _sil_orphan_label_key(raw_label)
        ...
        if g is None:
            g = {
                ...
                # 專屬欄位，確認後改為要在畫面上新增獨立可見欄位，比照notes/order_nos的
                # 收集+合併寫法。
                'techkt_notes': [], 'techkt_reconcile_orders': [], 'techkt_reconcile_nos': [],
                # 2026-09-11新增：Stacy要求原本「只用於搜尋/去重、不顯示」的通用
                # reconcile_order/reconcile_no/bookstore_reconcile_no這3個欄位也要顯示成
                # 獨立欄位，比照上面techkt_*三欄同一套收集+合併寫法。
                'reconcile_orders': [], 'reconcile_nos': [], 'bookstore_reconcile_nos': [],
                'values': {},
        ...
        # 2026-09-11新增：見上方group初始化註解
        if r.get('reconcile_order') and r.get('reconcile_order') not in g['reconcile_orders']:
            g['reconcile_orders'].append(r.get('reconcile_order'))
        if r.get('reconcile_no') and r.get('reconcile_no') not in g['reconcile_nos']:
            g['reconcile_nos'].append(r.get('reconcile_no'))
        if r.get('bookstore_reconcile_no') and r.get('bookstore_reconcile_no') not in g['bookstore_reconcile_nos']:
            g['bookstore_reconcile_nos'].append(r.get('bookstore_reconcile_no'))
    ...
    for gkey in order_seq:
        ...
        rows.append({
            ...
            # 2026-09-11新增：見上方group初始化註解
            'reconcile_order': '、'.join(g['reconcile_orders']),
            'reconcile_no': '、'.join(g['reconcile_nos']),
            'bookstore_reconcile_no': '、'.join(g['bookstore_reconcile_nos']),
            'values': g['values'],
            'row_total': row_total,
...
def invoice_ledger_wide_view_export():
    ...
    # 2026-09-11新增：對帳順序/對帳編號/書店對帳編號也比照辦理，加入匯出欄位。
    # 2026-09-11改名（使用者事後修正過一輪，以此為準，僅顯示文字，底層資料欄位/邏輯不變）：
    # 期間→對帳月份、期間區間→對帳區間、備註→對帳備註、書店對帳編號→已認列編號、
    # 科技通備註→公司別(2024/7後變更)、科技通對帳順序→出貨對帳順序、科技通對帳編號→出貨對帳編號
    base_headers = ['客戶類型', '對帳名稱', '對帳月份', '對帳區間', '對帳備註',
                     '對帳順序', '對帳編號', '已認列編號', '對帳訂單編號',
                     '公司別(2024/7後變更)', '出貨對帳順序', '出貨對帳編號']
    ...
    for row in rows:
        vals = [row['customer_type'], row['reconcile_name'], row['period'], row['period_range'],
                row['note'], row['reconcile_order'], row['reconcile_no'], row['bookstore_reconcile_no'],
                row['order_no'],
    ...
    sub_row = ['小計（共%d列）' % len(rows)] + [''] * (len(base_headers) - 1)
    ...
    # 2026-09-11修正：base_headers從9欄擴充為12欄（新增對帳順序/對帳編號/書店對帳編號3欄，
    # 插在備註之後、對帳訂單編號之前），原本寫死A~I這幾個欄位字母的寬度設定要跟著往後移，
    # 否則新插入的3欄會沒有寬度設定、後面的科技通3欄寬度也會套到錯的欄位上。
    ws.column_dimensions['A'].width = 14   # 客戶類型
    ws.column_dimensions['B'].width = 18   # 對帳名稱
    ws.column_dimensions['C'].width = 10   # 對帳月份
    ws.column_dimensions['D'].width = 10   # 對帳區間
    ws.column_dimensions['E'].width = 18   # 對帳備註
    ws.column_dimensions['F'].width = 12   # 對帳順序
    ws.column_dimensions['G'].width = 12   # 對帳編號
    ws.column_dimensions['H'].width = 14   # 已認列編號
    ws.column_dimensions['I'].width = 16   # 對帳訂單編號
    ws.column_dimensions['J'].width = 18   # 公司別(2024/7後變更)
    ws.column_dimensions['K'].width = 14   # 出貨對帳順序
    ws.column_dimensions['L'].width = 14   # 出貨對帳編號
    for i in range(len(columns)):
...
def _material_next_seq(prefix):
    ...
    rows = sb.table('material_master_list').select('material_code') \
        .ilike('material_code', prefix + '%').execute()  # 2026-09-08修正：自製supabase_client.py沒有實作.like()，只有.ilike()，原本的.like()一律會拋AttributeError導致新增100%失敗
    max_seq = 0
...
def create_material():
    # 2026-09-08新增：外層加一層try/except包住整個函式（含_material_next_seq()查詢），
    # 避免這段查詢萬一噴出未預期例外時，Flask直接回傳非JSON的500錯誤頁，導致前端
    # res.json()解析失敗被靜默吞掉、使用者只看到完全沒有細節的空白「新增失敗」——
    # 改成一律回傳JSON詳細錯誤訊息（含例外類型+訊息)，並把完整traceback印到Render
    # log方便日後排查（實測發現：直接用SQL Editor手動INSERT一模一樣的資料是成功的，
    # 代表問題出在這支API本身、不是資料庫，需要這層才能看到真正原因）。
    try:
        data = request.json or {}
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'error': f'新增失敗（伺服器錯誤 {type(e).__name__}）：{e}'}), 500
...
def _sku_id_name_code_maps():
    """回傳(name_by_id, code_by_id)：id->目前財報料號品名/id->目前內部料號，供「銷退清單」
    顯示時id優先反查目前主檔文字用（2026-09-02一勞永逸方案，比照書店拆帳引擎
    的name_by_id寫法，見_compute_bookstore_order_matrix()）。"""
    sku_rows = _fetch_all_rows(lambda: sb.table('internal_sku_list').select('id,report_product_name,internal_code'))
    name_by_id = {s['id']: s['report_product_name'] for s in sku_rows if s.get('id') and s.get('report_product_name')}
    code_by_id = {s['id']: s['internal_code'] for s in sku_rows if s.get('id') and s.get('internal_code')}
    return name_by_id, code_by_id


def _sales_return_row_with_items(row, items, name_by_id=None, code_by_id=None):
    """2026-09-02修正：財報料號品名/子料號一律id優先反查「現在」的主檔文字，查不到id
    （舊資料尚未回填、或id對應的料號已被刪除）時才退回用當初存的文字快照顯示，避免料號
    改名後這裡繼續顯示改名前的舊品名。"""
    name_by_id = name_by_id or {}
...
def list_sales_return():
    ...
    name_by_id, code_by_id = _sku_id_name_code_maps()
    out = [_sales_return_row_with_items(r, items_map.get(r['id'], []), name_by_id, code_by_id) for r in rows]
    return jsonify(out)
...
def export_sales_return():
    ...
    if return_ids:
        ...
        for it in items_rows:
            items_map.setdefault(it['return_id'], []).append(it)
    name_by_id, _code_by_id = _sku_id_name_code_maps()
    ...
    for i, r in enumerate(rows, 1):
        ...
        names = '、'.join((name_by_id.get(it.get('report_product_name_id')) or it.get('report_product_name') or '') for it in items)
        ws.append([
...
def get_sales_return(rid):
    ...
    name_by_id, code_by_id = _sku_id_name_code_maps()
    return jsonify(_sales_return_row_with_items(res.data[0], items_res.data or [], name_by_id, code_by_id))
...
def create_sales_return():
    ...
    # 2026-09-02修正：一勞永逸方案——存檔時一併把使用者選的財報料號品名/子料號文字解析成
    # id存起來（report_product_name_id/sub_internal_code_id），比照bookstore_plan_items
    # 的寫法，見_valid_report_names_to_id()/_valid_sku_codes()說明。
    name_to_id = _valid_report_names_to_id()
    code_to_id = _valid_sku_codes()
    item_rows = [{
        ...
        'amount': _to_num(it.get('amount')), 'sub_internal_code': _norm(it.get('sub_internal_code')) or None,
        'report_product_name_id': name_to_id.get(_norm(it.get('report_product_name'))),
        'sub_internal_code_id': (code_to_id.get(_norm(it.get('sub_internal_code')))
                                  if _norm(it.get('sub_internal_code')) else None),
    } for it in items]
